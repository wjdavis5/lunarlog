...
`COORDINATOR_ID` coordinator (its id is injected at the top of this prompt as
`COORDINATOR_ID=<id>`; use it wherever this document writes `COORDINATOR_ID`).
...
2. Your second action is `git rev-parse --show-toplevel`. If the output is not the worktree path
   ...
   directory.
3. Your third action is to confirm the toplevel path contains `.worktrees/COORDINATOR_ID/`. If it
   does not — any other location, including another coordinator's worktree layout — stop
   immediately and return "WRONG WORKTREE: not under .worktrees/COORDINATOR_ID/" with the path. You
...
6. Confirm the issue is claimed before you begin. Run `gh issue view <issue> --json labels` and
   verify BOTH `in-progress` and `owner:COORDINATOR_ID` are present. If either is missing, stop
...
12. Open the PR with:
    `gh pr create --fill --label owner:COORDINATOR_ID --label in-progress --body "<template from the brief, filled in>"`.
    The two `--label` flags are mandatory on every PR you open — `owner:COORDINATOR_ID` is this
