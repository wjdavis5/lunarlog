...
1. Record the PR's head SHA (`gh pr view <pr> --json headRefOid`) — this is the reviewed head: every step below examines this SHA and the merge in step 7 pins to it, so a push landing after review makes the merge refuse rather than merging unreviewed commits. Then `gh pr checks <pr> --watch`. Failing CI → request changes with the failing output → step 8.
