...
void main() {
  testWidgets(
    ...
    expect(
      find.byKey(ValueKey('household-changes-$barbId')),
      findsOneWidget,
    );
    expect(
    ...
    // shows silence alone.
    expect(
      find.byKey(ValueKey('household-timing-$aliceId')),
      findsNothing,
    );
    expect(
  ...
  testWidgets(
      ...
      'exactly as before (AC behaviour unchanged)', (tester) async {
    final db = await _pumpHousehold(
      tester,
      seed: (db) async {
    ...
    expect(find.text('Alice'), findsOneWidget);
    final aliceId = (await DriftProfilesRepository(db.storage).list()).single.id;
    expect(
      find.byKey(ValueKey('log-today-$aliceId')),
      findsNothing,
    ...
    expect(
      find.byKey(ValueKey('household-changes-$aliceId')),
      findsNothing,
    );
    await _disposeHousehold(tester, db);
  });
  ...
  testWidgets(
      ...
      'dismissal returns to the household view (AC)', (tester) async {
  ...
  testWidgets(
    ...
    final db = await _pumpHousehold(
      ...
      seed: (db) async {
        final alice = await _profile(db, 'Alice');
        final clare =
            await _profile(db, 'Clare', mode: ProfileMode.irregular);
        aliceId = alice.id;
        clareId = clare.id;
    ...
    final standardLine = tester
        ...
        .data!;
    expect(standardLine, '4 days late');
  ...
  testWidgets(
      ...
      '(constraint: a viewer sees status, not the action)', (tester) async {
    final auth = FakeAuthService(
      initialState: AuthSessionState.signedIn,
      user: const AuthUser(id: 'user-me', email: 'me@example.com'),
    );
  ...
  testWidgets(
      'content discretion: a note body and a symptom label are never '
