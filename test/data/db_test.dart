...
void main() {
  ...
  group('schema', () {
    ...
    test('schema version is 29 and database opens with the expected tables',
        () async {
      ...
      expect(await columnsOf(db, 'profile_modes'),
          containsAll(['profile_id', 'mode', 'mode_started_on', 'estimated_due_date', 'postpartum_birth_date',
      ...
      expect(await columnsOf(db, 'health_export_ledger'),
          containsAll(['record_id', 'profile_id', 'source_row_id', 'kind',
              'local_date', 'exported_at']));
  ...
  group('migrations', () {
    ...
    test('a database already at the latest version does not re-run the '
      ...
      expect(await userVersion(second), 29);
