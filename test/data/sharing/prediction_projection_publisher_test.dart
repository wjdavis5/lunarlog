...
ActivePrediction _active(LocalDate today) {
  // The forecast must carry at least one cycle whose bleed band lands
  // strictly after `today` — buildPredictionProjection's forward-only rule
  // (matching forecast.dart's own rendering) drops anything else, so an
  // empty (default) forecast would leave periodDays empty and defeat these
  // tests' whole point.
  final nextStart = today.addDays(6);
...
void main() {
  ...
  test('publishNow publishes immediately for a connected profile', () async {
    // publishNow never touches activeProfiles and this test never calls
    // publisher.start(), so activeProfiles is a plain empty stream rather
    // than a StreamController: a single-subscription StreamController's
    // `.close()` future only completes once some listener has subscribed
    // and cancelled -- with none ever subscribing, awaiting `.close()` in
    // teardown would hang forever (a well-known dart:async pitfall, not a
    // publisher bug).
    const activeProfiles = Stream<List<Profile>>.empty();
    final predictions = <String, StreamController<CyclePrediction>>{};
    final service = _FakeService(connectedProfileIds: {'p1'});
    final today = LocalDate(2026, 8, 30);
    ...
    // Create the controller before publishNow subscribes: a single-
    // subscription controller buffers the event, so the `.first` read
    // inside publishNow still sees it.
    predictions.putIfAbsent('p1', () => StreamController<CyclePrediction>(sync: true));
    predictions['p1']!.add(_active(today));
