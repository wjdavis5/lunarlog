...
void main() {
  ...
  group('gating (KTD10)', () {
    ...
    test('a locked gate at trigger time yields paused without any transport '
      ...
      rig.gate.unlock();
      await rig.engine.flush();
      expect(rig.transport.pullCount, 11); // #522 adds deleted_profiles, #130 adds day_entry_merge_events, #257 adds profile_tag_registry
      expect(rig.engine.snapshot.phase, SyncPhase.idle);
    });
    ...
    test('the periodic timer requests a sync', () async {
      ...
      await rig.engine.flush();
      expect(rig.transport.pullCount, pulls + 11); // #522 adds deleted_profiles, #130 adds day_entry_merge_events, #257 adds profile_tag_registry
    });
  });
}
