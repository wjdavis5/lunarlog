...
- **Database tests (`supabase/tests/`):** 186 pgTAP tests (`000-setup.sql` 1, `rls_isolation_test.sql` 52, `sync_push_test.sql` 82, `profile_guardians_rls_test.sql` 28, `guardian_sync_push_test.sql` 23). The `basejump/supabase_test_helpers` API subset the tests use is inlined in `000-setup.sql`, so the suite needs no network access to `dbdev`.
