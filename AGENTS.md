...
- **Database tests (`supabase/tests/`):** 231 pgTAP tests (`000-setup.sql` 1, `rls_isolation_test.sql` 51, `sync_push_test.sql` 87, `profile_guardians_rls_test.sql` 35, `guardian_sync_push_test.sql` 26, `guardian_revocation_bypass_test.sql` 7, `feedback_rls_test.sql` 20, `feedback_attachments_rls_test.sql` 4 — its plan's "delete own object" case does not run here; `storage.protect_delete()` blocks every direct SQL DELETE against `storage.objects` in this stack, see the file's header comment). The `basejump/supabase_test_helpers` API subset the tests use is inlined in `000-setup.sql`, so the suite needs no network access to `dbdev`.
