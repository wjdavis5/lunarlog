...
Implementation plans of record: [`docs/plans/2026-09-02-001-feat-supabase-auth-cloud-sync-plan.md`](docs/plans/2026-09-02-001-feat-supabase-auth-cloud-sync-plan.md) (accounts and sync), [`docs/plans/2026-09-03-001-feat-social-logins-plan.md`](docs/plans/2026-09-03-001-feat-social-logins-plan.md) (Google, passwordless email, identity linking; its IDs are cited with a `#2` prefix), and [`docs/plans/2026-09-05-001-feat-account-deletion-and-json-export-plan.md`](docs/plans/2026-09-05-001-feat-account-deletion-and-json-export-plan.md) (in-app account deletion and JSON export, the App Store 5.1.1(v) release-gate clearer; its IDs are cited with a plain `#17` prefix). Operational go-live, device, and release checklists: [`docs/ops/supabase-go-live.md`](docs/ops/supabase-go-live.md).
...
4. Merge to `main`: [`.github/workflows/supabase-migrate.yml`](.github/workflows/supabase-migrate.yml) runs `supabase link`, `db push --dry-run`, and `db push` inside the `production` GitHub environment and waits for the required reviewer, then (issue #17) sets the `delete-account` function's `APPLE_*` secrets and deploys it. A change under `supabase/functions/` is proven first with local `supabase functions serve` + `curl` (`edge-runtime` must **not** be excluded from the `start` command for that - see `docs/ops/supabase-go-live.md`'s "delete-account Edge Function runbook"), since there is no automated Deno coverage in CI yet.
...
- **Device checklist:** auth links, Apple and Google Sign-In, magic links and emailed codes, adding a sign-in method, two-device sync convergence, lock-mid-sync, the sign-out reset, account deletion (including the real Apple revocation), and JSON export cannot be covered by `flutter test`; they are verified by hand on iPhone and Android builds with a throwaway account per [`docs/ops/supabase-go-live.md`](docs/ops/supabase-go-live.md) ("Device checklist"). Use fabricated profiles only — never real family data.
- **Release gate:** cleared 2026-09-05 (issue #17) — in-app account deletion shipped, so App Store guideline 5.1.1(v)'s submission block (deletion required once account creation exists) no longer applies. `pubspec.yaml`'s `version:` bump and a `submit_for_review` dispatch of `ios-release.yml` remain a separate, deliberate release action, not automatic from this change. Pushes to `main` still upload to TestFlight and the Play `internal` track; that is acceptable for internal testing only.
