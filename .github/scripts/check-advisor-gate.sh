...
#     issue #194's pg_net relocation is deferred, not solved: production's
#     pg_net is supabase_admin-owned and non-relocatable (`alter extension
#     ... set schema` fails, confirmed locally, SQLSTATE 0A000), and its
...
#     Supabase-managed grants, which push dispatch depends on. This needs a
#     Supabase-supported relocation path (not a plain migration) -- tracked
#     as a follow-up, not solved here. ANY OTHER extension_in_public
#     finding (a different extension, or pg_net misreported under some
#     other metadata shape) still fails.
