...
- iOS: the database file now lives under `getApplicationSupportDirectory()`,
  ...
  `Library/Caches/` are excluded automatically. The directory move is
  hygiene, not the control), with a one-time crash-safe startup migration
  that relocates a pre-#244 install's `Documents/lunarlog.db` (and its
  `-wal`/`-shm`/`-journal` siblings) the first time it runs
  (`lib/startup/database_relocation.dart`'s `relocateLegacyDatabase` —
  stages every sibling, verifies the staged copy against the legacy
  database, then atomically renames each into place and writes a
  completion sentinel last; any failure rolls back the partial target and
  falls back to opening the legacy file). The actual backup control is
  `AppDelegate.swift`'s `lunarlog/privacy` channel (the iOS half of the
  ...
  analyze`/`flutter test`/`flutter build apk` all pass, but the iOS-only
  platform-channel code cannot run under `flutter test` and this repo's
  Windows dev box cannot build iOS). There is no app-managed database key
  to pin or unpin now that at-rest protection is the OS's own
