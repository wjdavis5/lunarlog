...
### Update — third wave, 2026-09-09 (later)

Merged (issue → PR): #223 → #315 (Analysis content on the Insights tab: headline statistics from `ActivePrediction`, cycle history mounted; review caught a viewer-role write path and two disagreeing statistics on one screen); #209 → #316 (Today card, cycle wheel, one-tap "period started today" with undo and disclosure of the medium-flow default, shell FAB; review caught a silent write failure and a colour-only predicted band); #190 → #318 (Clue export parser and mapping spec — issue left open for four real-export checks; review caught a parse error that would have embedded a fragment of the user's export in an exception); #159 → #320 (import provenance columns, partial unique indexes, re-emitted `sync_push`, Drift v7, export v4; review caught the day sheet wiping provenance on Save); #314 → #322 (cycle history lives only on the Analysis tab; `AppShellScope` tab-switch seam; archived profiles keep a read-only card); #180 → #319 (time-zone contract: `is_valid_timezone` CHECKs, `local_date` derivation trigger, day-boundary helpers; review caught that `sync_push`'s per-day cap and dedup ran before the trigger rewrote the date, so the derivation now lives in `sync_push` itself); #308 → #323 and #312 → #324 (component and calendar follow-ups); #313 → #330 (Android back to Today, named sharing routes, Activity Feed in the shell bar); #152 → #333 (verified LOINC set and SNOMED/local dual coding for the existing tags; review independently verified every concept and corrected one LOINC display); #122 + #150 → #334 (product repositioning across README, PRIVACY, the hosted policy, in-app copy).

In review or CI at the time of writing: #325 (issue #140, import of the app's own export — two review rounds so far), #327 (issue #167, `bulk_import_entries` RPC and `import_jobs`), #329 (issue #321, notification time-zone CHECK), #335 (issue #247, flow model: superHeavy, explicit notBleeding, spotting as an observation — review found the caregiver-alert trigger would treat "not bleeding" as a period start, fixed on top of #327's body).

Dispatched: #143 (fertile-window estimation), #157 (FHIR R4 document Bundle).

Further follow-ups filed: #321 (time-zone contract follow-ups), #326 (EmptyState follow-ups), #328 (observations bulk import), #331 (client handling of a rejected time zone), #332 (same-date resolver inside the bulk RPC).

Human steps recorded: #190 (obtain one real Clue export to pin four fields), #140 (file-picker manifest permission and store questionnaire), #180 (time-zone pre-flight query before `db push`), #247 (spotting backfill runs against live data — take a row count first), #152 (the permanent FHIR local-system URI), #209 (the medium-flow quick-log default).

Process notes: branch protection requires the six checks on the PR's current head, so every rebase needs a fresh CI run (the "file-disjoint drift, skip CI" shortcut is refused); three coders stalled on an infrastructure watchdog when six agents ran concurrently — four is the safe ceiling, and a finisher can continue from the stalled worktree since coders commit-or-leave work in place; the `git checkout --ours` rebase trap and the CRAP-gate arithmetic (uncovered handlers must be ≤ 2 branches) are now in the coder brief.

### Update — fourth wave, 2026-09-09 (close-out)

The operator asked for no further dispatches after this wave; the in-flight branches were driven to merge and the run closed out.

Merged (issue → PR): #143 → #337 (calendar-method fertile-window and ovulation estimate with confidence tiering); #157 → #338 (FHIR R4 document Bundle, IPS-shaped, exported on-device through the share sheet; the export filters on the positive bleed allow-list so an explicit "not bleeding" day never emits a menstrual-status Observation); #197 → #339 (month-window day-entry subscription, sqlite indexes under Drift schema v8, memoised prediction recompute; the last fix wired the new l10n delegates into a windowed-subscription test after #340 made the calendar read `AppLocalizations`); #247 → #335 (flow model: `superHeavy`, explicit `notBleeding`, spotting demoted to an observation with a backfill; `sync_push`, `enqueue_caregiver_alerts`, and `bulk_import_entries` re-emitted over #327's bodies; export schema v5); #321 → #329 (`is_valid_timezone` CHECK on `notification_preferences.time_zone`, `resolve_deliver_after` defence removed, PRIVACY time-zone note).

What the last CI rounds on #335 taught: a pgTAP scenario that narrows a caregiver's alert preference must run the `update` as that caregiver — the `notification_preferences` update policy is self-only, so as the profile owner the statement matched zero rows and silently left the earlier preference in force (the suite now asserts the narrowed row before relying on it); `alert_on_log` is the master switch in `enqueue_caregiver_alerts`, so "high-severity only" means `alert_on_high_severity = true` with `alert_on_log` left on; a per-kind outbox count (via a `pg_temp` SECURITY DEFINER helper) is the assertion that survives earlier scenarios' rows, not a total; and the CRAP gate fails at exactly 10.0, so a complexity-10 widget body with one uncovered branch has to be split (`_editableBody` → `_flowChips`).

Process notes: `pr-wait2.sh` (wait for the ci.yml run whose head SHA equals the PR head) replaced the older poller, which attributed the previous head's run to a just-pushed one; one chain still read the PR head before the push propagated and waited on a SHA that never ran — read the head after a short delay. Squashing a multi-commit branch to one commit before the final rebase keeps every conflict in a single resolution pass. With zcode merging to main in parallel (#340 l10n scaffolding, coordinator state commits), every branch needed at least two rebases between review and merge; ordering the merges by drift (smallest first) kept that to one extra CI run per PR.

Open at close-out: #341 and #342 are zcode's, untouched. Worktrees `orch-247`, `orch-197`, and `orch-321` were removed by the finish step and their `in-progress` labels cleared. Human steps recorded on the issues stand as listed in the third-wave update.
