...
| `opencode-deepseek` | OpenCode (`deepseek/deepseek-v4-flash`) | `opencode-deepseek/<n>-<slug>` (`opencode-deepseek/fix-<n>-<slug>` for bugs) | `.worktrees/opencode-deepseek/<n>-<slug>` | [`docs/coordinator/opencode-deepseek/`](opencode-deepseek/) |
...
model exists, like `opencode-deepseek`, gets a clean per-id directory from day one and has no
...
| `owner:opencode-deepseek` | Owned by the `opencode-deepseek` coordinator. |
