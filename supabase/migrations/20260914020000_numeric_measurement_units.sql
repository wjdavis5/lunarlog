-- Migration: 20260914020000_numeric_measurement_units.sql
...
--   5. `observations_live_profile_date_category_code_uq` (#565's
--      structural dedup backstop) is dropped and re-created with `source`
--      in the key -- without this, the index (which postdates the branch
--      this migration was authored on) would reject cross-source siblings
--      at INSERT time and silently defeat item 3.
--
...
--
-- Filename ordering (AGENTS.md Migration Flow item 7): originally
-- 20260909210000_, then 20260911090000_ (dodging the
-- 20260909210000_profile_transferred_to_user_id.sql collision), now
-- 20260914020000_ because main has since gained migrations through
-- 20260914010000 (#643's observation reparenting). The `sync_push` body
-- below is carried forward from 20260914010000_sync_push_observation_reparent.sql
-- (#643, main's current definition -- including its same-date observation
-- reparenting, which composes with item 3's source-scoping: reparenting
-- moves rows whole without deduping, and the observations loop's dedup
-- still runs per surviving row) other than the additions called out
-- inline; `export_account_data()`'s body is carried forward from
-- 20260908190000_bulk_import.sql (main's current definition) plus item 4's
-- projection lines. The signature is unchanged in both cases, so plain
-- create-or-replace applies (no overload fork).
...
-- ---------------------------------------------------------------------------
-- 3. The dedup's structural backstop is re-keyed to match the source-scoped
-- dedup below: #565's `observations_live_profile_date_category_code_uq`
-- (20260913018000) predates this migration's source discipline and would
-- reject a cross-source live sibling at INSERT time (23505 -> the per-row
-- exception handler -> a rejected row), silently defeating section 4's
-- widened SELECT. `source` is `not null default 'manual'` (#240), so the
-- widened key never sees a NULL; the race #565 closed was always
-- manual-vs-manual (same source), which still violates the re-keyed index
-- exactly as before.
-- ---------------------------------------------------------------------------

drop index if exists public.observations_live_profile_date_category_code_uq;

create unique index observations_live_profile_date_category_code_uq
  on public.observations (profile_id, local_date, category, code, source)
  where deleted_at is null and code is not null;

comment on index public.observations_live_profile_date_category_code_uq is
  'Issue #565, re-keyed by #255: structural backstop for the same-date
   (category, code) dedup sync_push''s own head-to-head resolution performs,
   now source-scoped like the dedup itself -- a wearable-sourced value and a
   manually-entered value at the same (profile, date, category, code) are
   different facts and coexist; the race this closes was always
   manual-vs-manual (same source), which still violates this index.';

-- ---------------------------------------------------------------------------
-- 4. sync_push: the two keys join the profile allowlist, the parse block,
...
-- ---------------------------------------------------------------------------
-- 5. export_account_data: the two new profile columns join the export
-- projection (right-of-access completeness). Body carried forward verbatim
-- from 20260908190000_bulk_import.sql other than the two lines called out
-- inline.
-- ---------------------------------------------------------------------------
