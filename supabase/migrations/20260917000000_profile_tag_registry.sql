-- Migration: 20260917000000_profile_tag_registry.sql
--
-- Issue #257 (P1, epic: tracking-model): "feat(logging): per-profile
-- custom-tag registry". lunarlog's tag vocabulary has been entirely static
-- (`lib/domain/tags.dart`'s kTagTaxonomy): the client write path rejected
-- any code outside it (`validateTagCodes`'s ArgumentError), the day-sheet
-- picker rendered only taxonomy chips, and a Clue user's free-text custom
-- tags -- often the most personally meaningful part of their record --
-- had nowhere to go, which by itself blocks a lossless Clue import
-- (#257's A1-36/D-9 framing). The server, meanwhile, has always treated
-- `day_entries.tags`/`observations.code` as free text bounded by
-- `is_valid_tags_array` (<=32 elements, <=64 chars each) -- it never knew
-- what a code *means*.
--
-- This migration adds `public.profile_tag_registry`: the per-profile,
-- synced, user-owned custom-tag vocabulary -- the ELEVENTH synced table,
-- following the exact per-table pattern issue #130's
-- day_entry_merge_events just set end to end:
--
--   1. The table: one row per custom tag per profile. `code` is the same
--      shape as a taxonomy code (stable snake_case identifier, stored on
--      day entries); `display_name` is the user's own label (bounded to
--      40 chars by CHECK); `category` is free text the client owns
--      ('custom' for tags created in-app today). `hidden_at` is
--      RETIREMENT: it removes a code from the day-sheet picker while
--      stored rows referencing the code keep rendering -- Clue's
--      delete-destroys-history failure mode (a documented user complaint)
--      is made structurally impossible: there is no code path that can
--      remove a registry row's identity, and even the tombstone path
--      (deleted_at) keeps `code` so a future re-import can recognise the
--      deliberate deletion instead of resurrecting it (the #159
--      provenance-survives-tombstone precedent).
--
--   2. Dedupe: a PARTIAL unique index on `(profile_id, lower(code))
--      where deleted_at is null` -- case-insensitive (the issue's exact
--      ask), partial so a tombstoned row never blocks re-creating the
--      same code later (the day_entries live-unique-index precedent; a
--      plain UNIQUE constraint, as the issue sketched it, would freeze a
--      deleted code forever).
--
--   3. Per-profile cap of 100 live rows, enforced in `sync_push`'s write
--      path -- NOT a CHECK, because a CHECK cannot count sibling rows
--      (the same reasoning as #240 (TM-2)'s per-day observation cap).
--
--   4. RLS mirrors `day_entries`' guardian-role predicates exactly: any
--      accepted guardian selects; writes have NO policy and NO grant at
--      all -- `sync_push` (SECURITY DEFINER, the sole write path since
--      20260915160000) is the only writer, with the day_entries write
--      ladder (primary_guardian / co_parent / caregiver; viewer
--      read-only). A custom tag is logging vocabulary, not profile
--      metadata -- the #128 care-notes ladder, not the stricter #188
--      profile-metadata ladder.
--
--   5. `sync_push` is re-emitted from its immediate predecessor
--      (20260916100000_day_entry_merge_events.sql, still the latest
--      definition on main at dispatch time) with exactly the #257
--      changes: a ninth defaulted payload parameter `p_tag_registry`
--      (the #240/#188/#128/#130 defaulted-param pattern; the 8-arg
--      overload is dropped first so exactly one sync_push exists), its
--      key allowlist + `v_role_map` union + per-row loop (per-id LWW
--      with tombstones, the care_notes shape: cannot move between
--      profiles, `?` containment guards on the optional payload columns
--      so a partial-row writer never clobbers a stored value,
--      unconditional payload clears on the tombstone branch), and the
--      per-profile live-row cap. `c_max_total_rows` grows 4000 -> 4500
--      (= 9 * c_max_rows), keeping the combined cap a no-op for nine
--      arrays (the #564 discipline). Everything else -- the 20260915200001
--      tombstone-resurrection guard and #130's resolver emissions
--      included -- carries forward unchanged.
--
--   6. `created_by` is stamped by the server from the caller on INSERT
--      and never accepted per-row (absent from the key allowlist, the
--      #128 checked_by/checked_at pattern: a client can never forge
--      "who created this tag"). `created_at` is likewise server-stamped
--      and tolerated-but-never-read.
--
--   7. `sync_pull(p_cursors)`: a tenth per-profile key,
--      'profile_tag_registry', same tenant scoping and page contract as
--      every content table.
--
--   8. `tombstone_profile_content()`: the profile's registry rows are
--      TOMBSTONED (not hard-deleted -- unlike #130's merge events, these
--      rows are ordinary synced content whose deletion must propagate to
--      every co-guardian device, and the tombstone is what carries it),
--      payload cleared per the table's structural CHECK, server_version
--      bumped by the trigger so the wipe propagates via the ordinary
--      incremental pull. Counted in the returned jsonb.
--
--   9. `reconcile_realtime_publication()`: the table joins the
--      must-never-publish list (a custom tag's display name is
--      user-authored health vocabulary about a potentially minor
--      profile). A `touch_sync_signal()` trigger still gives
--      co-guardians' devices a content-free realtime wake, exactly like
--      every content table.
--
--   10. `export_account_data()` is deliberately untouched, on the
--      #188/#128/#130 precedent (client-synced tables are the local JSON
--      export's responsibility). `enforce_retention()` is untouched too:
--      registry rows are deliberate configuration with no retention
--      window -- they live until retired (hidden_at), tombstoned, or
--      their profile is wiped.
--
-- Design decisions worth naming:
--
--   * Retirement, never deletion, is the PRIMARY lifecycle: `hidden_at`
--     is the UI's "remove this tag from my picker" write, and it leaves
--     every stored day_entries.tags/observations.code reference
--     rendering. `deleted_at` (the synced-table tombstone) exists for
--     genuine data removal (a profile wipe, or a future "delete this
--     tag's registry entry outright" flow), and even it keeps `code`.
--   * `hidden_at` is payload, cleared with everything else on a tombstone
--     (the tombstone payload CHECK forces it null): a tombstone is always
--     strictly "more deleted" than retired, never a resurrection.
--   * The registry NEVER gates writes: an observations/day_entries row
--     referencing a code absent from the registry (never created,
--     retired, not yet synced to this device, or deleted) is as valid
--     as it ever was -- the server's free-text treatment of codes is
--     unchanged (#237 (TM-1)'s unknown-never-drop rule, preserved by
--     construction: nothing in this migration validates any code against
--     this table). The registry is display vocabulary, never an
--     allowlist.
--
-- Coverage: supabase/tests/profile_tag_registry_test.sql.

-- ---------------------------------------------------------------------------
-- 1. public.profile_tag_registry
-- ---------------------------------------------------------------------------

create table public.profile_tag_registry (
  id text not null
    constraint profile_tag_registry_id_ulid_check
    check (id ~ '^[0-9A-HJKMNP-TV-Z]{26}$'),
  profile_id text not null
    references public.profiles (id) on delete cascade,
  -- The stable snake_case identifier, the same shape and the same
  -- per-element bound as a taxonomy code (`is_valid_tags_array`'s 64-char
  -- element ceiling; `lib/domain/limits.dart`'s kMaxTagLength mirrors it).
  -- NEVER validated against kTagTaxonomy or anything else server-side --
  -- the registry is display vocabulary, never an allowlist (header item
  -- 10). Survives a tombstone so a future re-import can recognise the
  -- deliberate deletion (the #159 provenance precedent).
  code text not null
    constraint profile_tag_registry_code_length_check
    check (char_length(code) between 1 and 64),
  -- The user's own label for the tag. Bounded to 40 characters by the
  -- issue's own CHECK (the display surface, not the stored identifier).
  display_name text not null
    constraint profile_tag_registry_display_name_length_check
    check (char_length(display_name) <= 40),
  -- Free text, client-owned ('custom' for in-app creations today; a
  -- future importer may file mapped Clue tags under their Clue category).
  -- Bounded to the same 64-char ceiling as code for symmetry, and
  -- required non-empty on a LIVE row -- but a tombstone clears it to ''
  -- (the #224 house pattern), so the two states are one constraint
  -- rather than a min-length CHECK the tombstone could never satisfy.
  category text not null
    constraint profile_tag_registry_category_check check (
      (deleted_at is null and char_length(category) between 1 and 64)
      or (deleted_at is not null and category = '')
    ),
  intensity_enabled boolean not null default false,
  -- RETIREMENT, not deletion: removes the code from the day-sheet picker
  -- while stored rows referencing it keep rendering (header item 1).
  hidden_at timestamptz null,
  sort_order integer,
  -- Stamped by the server from the caller on INSERT; never accepted from
  -- a payload (absent from sync_push's key allowlist -- the #128
  -- checked_by/checked_at pattern).
  created_by uuid,
  ...
  primary key (id),
  -- Tombstones carry no payload (the #224 house pattern): the label, the
  -- category, the retirement stamp and the sort position all clear.
  -- `code` is the one survivor -- the #159 provenance-survives-tombstone
  -- precedent, applied so a future re-import can tell "this code was
  -- deliberately deleted" apart from "never existed" (and the partial
  -- live-code unique index means the surviving tombstoned codes never
  -- collide with each other or with a re-created live row).
  constraint profile_tag_registry_tombstone_payload_check check (
    deleted_at is null or (
      display_name = ''
      and category = ''
      and intensity_enabled = false
...
-- Case-insensitive dedupe across LIVE rows only: partial so a tombstoned
-- row never blocks re-creating the same code later (the day_entries
-- live-unique-index precedent). An expression index, so it cannot be a
-- table constraint; the issue's `unique (profile_id, lower(code))`
-- intent is enforced exactly, minus the freeze-on-delete a plain UNIQUE
-- constraint would have introduced.
create unique index profile_tag_registry_live_code_uq
  on public.profile_tag_registry (profile_id, lower(code))
  where deleted_at is null;

create index profile_tag_registry_profile_idx
  on public.profile_tag_registry (profile_id);
create index profile_tag_registry_server_version_idx
  on public.profile_tag_registry (server_version);

comment on table public.profile_tag_registry is
  'Issue #257: the per-profile custom-tag registry -- one row per user-defined tag (code + display label), synced through sync_push/sync_pull under the day_entries guardian-role ladder (any accepted guardian reads; primary/co_parent/caregiver write via sync_push, the sole write path; viewer read-only). hidden_at is RETIREMENT (removed from the picker, stored rows keep rendering); deleted_at is the ordinary synced-table tombstone, payload-free per profile_tag_registry_tombstone_payload_check except `code`, which survives so a re-import can recognise the deletion. Case-insensitive dedupe per live row via profile_tag_registry_live_code_uq; at most 100 live rows per profile, enforced in sync_push (a CHECK cannot count siblings). The registry is display vocabulary, never an allowlist: codes on day_entries/observations remain free text.';

create trigger profile_tag_registry_set_server_version
  before insert or update on public.profile_tag_registry
...
create trigger profile_tag_registry_after_change_signal
  after insert or update or delete on public.profile_tag_registry
...
alter table public.profile_tag_registry enable row level security;
alter table public.profile_tag_registry force row level security;

create policy "profile_tag_registry_select_guardians" on public.profile_tag_registry
  for select to authenticated
  using (
    public.is_profile_guardian(profile_id, (select auth.uid()))
  );

revoke all on table public.profile_tag_registry from public, anon, authenticated;
grant select on table public.profile_tag_registry to authenticated;

-- ---------------------------------------------------------------------------
-- 3. sync_push: re-emitted verbatim from its immediate predecessor
--    (20260916100000_day_entry_merge_events.sql, still the latest
--    definition on main at dispatch time) with exactly the Issue #257
--    changes its own comments name -- the ninth defaulted payload
--    parameter + its loop + key allowlist + v_role_map union + the
--    per-profile live-row cap, and the c_max_total_rows 4000 -> 4500
--    no-op-cap growth. Everything else (the 20260915200001
--    tombstone-resurrection guard and #130's resolver emissions included)
--    carries forward unchanged.
-- ---------------------------------------------------------------------------


-- Postgres identifies a function by name + parameter TYPE LIST: adding
-- a parameter does not replace the existing 8-argument sync_push in
...
-- silently forking the function in two. Drop the 8-arg overload first
-- so exactly one sync_push exists afterwards and a 2- through
-- 8-argument call resolves to THIS body with p_tag_registry taking its
-- '[]'::jsonb default (the #240/#128/#130 technique).
drop function if exists public.sync_push(jsonb, jsonb, jsonb, jsonb, jsonb, jsonb, jsonb, jsonb);
...
declare
  ...
  c_merge_event_keys constant text[] := array[
    ...
    -- tolerated but never read
    'server_version'];
  -- Issue #257: profile_tag_registry's allowed keys. created_by and
  -- created_at are deliberately absent -- the server stamps both
  -- (created_by from the caller, exactly the way the #128 prep-item
  -- checked_by/checked_at exclusion works there), so a row carrying
  -- either is rejected as an unknown key rather than silently accepted,
  -- and a client can never forge "who created this tag".
  c_tag_registry_keys constant text[] := array[
    'id', 'profile_id', 'code', 'display_name', 'category',
    'intensity_enabled', 'hidden_at', 'sort_order', 'updated_at',
    'deleted_at',
    -- tolerated but never read
    'server_version'];
...
comment on function public.sync_push(jsonb, jsonb, jsonb, jsonb, jsonb, jsonb, jsonb, jsonb, jsonb) is
  ...
  'and profile_tag_registry under guardian role permissions with '
  'authoritative attribution stamping. '
  'See prior migrations'' function comments (20260906200000 onward) for the '
  'full per-table history; this comment covers only what '
  '20260917000000_profile_tag_registry.sql changed on top of '
  '20260916100000_day_entry_merge_events.sql. Issue #201 '
  ...
  'row whose tombstone was already hard-deleted. Issue #257: (a) a ninth '
  'defaulted payload parameter p_tag_registry accepts per-profile custom-'
  'tag registry rows under the day_entries write ladder (any accepted '
  'non-viewer guardian), per-id LWW with tombstones (the care_notes shape) '
  'plus cannot-move-between-profiles, `?` containment guards on the '
  'optional payload columns, and a 100-live-rows-per-profile cap counted '
  'in this RPC (a CHECK cannot count siblings); (b) created_by is stamped '
  'from the caller on INSERT and never accepted per-row; (c) '
  'c_max_total_rows grows from 4000 to 4500 to keep the combined cap a '
  'no-op for nine arrays. No other behavior change.';
...
revoke all on function public.sync_push(jsonb, jsonb, jsonb, jsonb, jsonb, jsonb, jsonb, jsonb, jsonb) from public, anon;
grant execute on function public.sync_push(jsonb, jsonb, jsonb, jsonb, jsonb, jsonb, jsonb, jsonb, jsonb) to authenticated;
-- ---------------------------------------------------------------------------
-- 4. sync_pull(p_cursors): re-emitted from 20260916100000 (unchanged
--    since) with the tenth per-profile key (see header item 7).
-- ---------------------------------------------------------------------------
...
comment on function public.sync_pull(jsonb) is
  ...
  'visit_prep_items, day_entry_merge_events, profile_tag_registry, '
  'profile_guardians -- filtered to the caller''s '
...
-- ---------------------------------------------------------------------------
-- 5. tombstone_profile_content(): re-emitted from its current definition
--    (20260916100000_day_entry_merge_events.sql, unchanged since) with
--    the registry tombstone step + count (header item 8).
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 6. reconcile_realtime_publication(): re-emitted from
--    20260916100000_day_entry_merge_events.sql (unchanged since) with
--    profile_tag_registry on the must-never-publish list (header item 9).
-- ---------------------------------------------------------------------------
...
begin
  ...
  if v_all_tables then
    raise exception
      ...
      'public.day_entry_merge_events, and public.profile_tag_registry '
...
comment on function public.reconcile_realtime_publication() is
  ...
  'day_entry_merge_events; Issue #257 added profile_tag_registry), drift '
