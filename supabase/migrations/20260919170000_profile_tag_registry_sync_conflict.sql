-- ===========================================================================
-- 20260919170000_profile_tag_registry_sync_conflict.sql
-- Issue #825: concurrent custom tag creation causes unresolvable sync_push
-- rejection loop.
--
-- Background:
--   When two devices concurrently create a custom tag with the same code
--   (case-insensitively), both generate distinct ULIDs locally. Device A
--   pushes first and succeeds, inserting its ULID under the partial unique
--   index profile_tag_registry_profile_code_uq. Device B pushes next with
--   a different ULID; sync_push finds no stored row for that id and
--   attempts an unconditional INSERT, raising unique_violation (23505).
--   The per-row exception handler marks the row rejected: true, leaving
--   the row dirty on device B and creating an infinite retry rejection loop.
--
-- Change:
--   Re-emit public.sync_push (signature unchanged) with deterministic
--   same-code collision resolution in the profile_tag_registry loop:
--     1. For live writes (v_deleted_at is null), query for an existing live
--        sibling under the same profile sharing lower(code) with id <> v_id
--        (for update).
--     2. If found, resolve via LWW (tiebreaker: v_id < sibling.id under
--        "C" collation):
--        - Incoming wins: tombstone the stored sibling at v_updated_at,
--          append the sibling to v_resolved, and let incoming insert/update
--          as live (the unique index is no longer violated).
--        - Incoming loses: convert the incoming row to a payload-free
--          tombstone at the winner's timestamp, append both the winning
--          sibling and the incoming tombstone to v_resolved, and store
--          incoming as a tombstone.
--     3. In both directions, 0 rejections occur, so device B marks the push
--        clean and receives the authoritative state.
--
-- Derived allowlists:
--   Follows AGENTS.md Migration Flow item 9 and copies the DO-block pattern
--   verbatim from 20260919160000_day_entry_date_bounds.sql.
-- ===========================================================================
...
begin
  ...
  -- The 20260918160000 statement verbatim except the ten array
  -- declarations (placeholder tokens) and the Issue #848 day_entry
  -- date-bounds edits (one new declared variable plus the live-branch
  -- check in the day_entries loop).
  v_body := $ddl$
