-- Migration: 20260907223248_restore_sync_push_tags_validation.sql
-- Fixes Issue #96: restores the public.is_valid_tags_array(v_tags) call that
-- 20260903170000_tags_string_array_check.sql added to sync_push and
-- 20260904020000_sync_push_and_invitations.sql silently dropped, leaving only
-- a jsonb_typeof type check. Per "do not edit a merged migration in place",
-- this create-or-replaces sync_push with only the tags-validation branch
-- changed, rebased onto 20260906200000_same_date_tag_merge.sql's body (the
-- current live definition) so the same-date tag union (issue #3 U4) and the
-- birth_year/relationship handling (issue #4 U1) are not reverted - exactly
-- the failure class #96 documents.
--
-- This is not merely contract tidiness: since 20260906200000 there is a live
-- laundering path. merge_tag_arrays unions tags via jsonb_array_elements_text,
-- which coerces non-string JSON scalars to their text form and caps the union
-- at 32. Because tags validation ran *before* the same-date resolver only as a
-- type check, an invalid tags value colliding with a live entry on the same
-- (profile_id, local_date) was silently rewritten into a valid one and landed
-- ([1, 2] became ["1", "2", ...]; a 33-element array was capped to 32). The
-- restored check runs before the resolver, so both the collision and
-- non-collision paths are rejected at the RPC, with day_entries_tags_check
-- kept as the second, independent layer behind it.
--
-- The raise carries no errcode, matching every sibling row-level validation in
-- this function (they are caught by the block's `when others` handler and
-- surface as the opaque per-row {"id": ..., "rejected": true}); critically,
-- no 22023 is added to the transient re-raise list, so a bad row still
-- rejects only its own row instead of aborting the batch. Tombstones are
-- untouched: they zero tags before this branch and '[]' is trivially valid.
-- No data backfill: 20260903170000 already sanitized rows at rest, and any
-- merge-path-laundered row is a valid string array by the time it is stored.
  ...
  for v_row in select value from jsonb_array_elements(p_day_entries) loop
    ...
    begin
      ...
      if v_deleted_at is not null then
        ...
        -- Issue #96: full string-array validation restored (was only a
        -- jsonb_typeof type check since 20260904020000). Runs before the
        -- same-date resolver below, so merge_tag_arrays can never coerce or
        -- cap an invalid value into validity on a collision.
...
comment on function public.sync_push(jsonb, jsonb) is
  'Batch upsert of profiles then day entries under guardian role permissions with authoritative attribution stamping. U1: profiles carries birth_year/relationship through this path; transferred_at is tolerated-but-never-read (ownership state, written only by accept_ownership_transfer). Same-date collisions union tags onto the surviving row (R7, issue #3 gap-closure plan U4) while flow/note stay last-writer-wins. Non-tombstone tags must be an array of at most 32 strings, validated by is_valid_tags_array inside the RPC itself (issue #96), before the same-date resolver runs.';
