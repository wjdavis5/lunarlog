-- Migration: 20260915200001_sync_push_tombstone_resurrection_guard.sql
--
-- Coordinator review of PR #705 (#263/#189), blocking finding 1: the newest
-- `sync_push` (20260915160000_sync_push_sole_write_path.sql) only rejects a
-- day_entries push whose `updated_at` is more than 5 minutes in the FUTURE
-- (Issue #566) -- it has no lower bound at all. Once
-- 20260915200000_nightly_retention_job.sql's `enforce_retention()`
-- hard-deletes a day_entries tombstone older than 180 days, that row's id
-- no longer exists server-side. A client that has been offline the entire
-- 180+ days (never learned of the deletion) and still holds the pre-delete
-- live row will, on its next push, find no stored row for that id and take
-- the INSERT branch -- resurrecting deleted health data about, potentially,
-- a minor. This closes that gap: the day_entries loop's "no stored row"
-- branch now rejects (not inserts) an incoming row whose `updated_at` is
-- older than the SAME 180-day window `enforce_retention()` purges on --
-- the two must stay in lockstep, which is why both are commented with the
-- identical "180 days" cross-reference rather than a derived SQL constant
-- (this repo's existing convention for cross-migration numeric agreement,
-- e.g. how `kSyncFullPullInterval` is referenced by comment only across
-- Dart and SQL).
--
-- `observations` does NOT need the equivalent guard: its loop never
-- inserts a day_entries row implicitly (checked line by line below,
-- unchanged by this migration) -- it looks up `day_entry_id` and raises
-- `'day_entry_id does not exist'` whenever no parent row is found, exactly
-- the rejection an orphaned/resurrected-and-then-rejected parent already
-- produces. Since the day_entries loop always runs before the observations
-- loop in the same call, a push that pairs a too-old-unknown day_entries
-- row with a companion observation for the same id rejects BOTH rows for
-- consistent reasons -- the day_entries row via this new guard, the
-- observation via its pre-existing parent-lookup check -- with no FK error
-- surfacing either way (the day_entries INSERT this guard skips is exactly
-- what would have satisfied observations' FK; skipping it just means the
-- observations lookup finds nothing, its own long-standing code path).
--
-- Re-emits `sync_push` verbatim from its immediate predecessor
-- (20260915160000_sync_push_sole_write_path.sql, still the latest
-- definition on `main` at dispatch time) with exactly one change: inside
-- the day_entries loop's `select * into v_stored ... if found then ... end
-- if;` block, a new `else` branch rejects an unknown id whose `updated_at`
-- predates the retention window (search "Coordinator review" in the body
-- below). `security definer`, `set search_path = ''`, and the function's
-- own `revoke`/`grant` are carried forward unchanged -- see
-- 20260915160000's own header for the DEFINER-safety verification, which
-- this migration does not revisit (it changes zero authorization logic).
-- The `revoke insert, update on table public.day_entries from
-- authenticated` from that migration is not repeated here: it already
-- landed and does not need re-applying.
--
-- pgTAP coverage added by this migration: supabase/tests/sync_push_tombstone_resurrection_guard_test.sql
-- (a stale (>180 days) push of an unknown id is rejected and not inserted;
-- a fresh push of an unknown id is inserted as before; a stale update to an
-- EXISTING row still follows ordinary LWW, unaffected by this guard, which
-- only ever fires on the "no stored row" branch).
--
-- Dart client: no change needed. `lib/data/sync/supabase_sync_engine.dart`
-- already treats every id present in `sync_push`'s `rejected` array
-- identically regardless of which server-side check produced it (it does
-- not inspect a reason string -- none is returned; see
-- 20260915160000's `v_rejected := v_rejected || jsonb_build_object('id',
-- v_row -> 'id', 'rejected', true)` sole shape, unchanged here) -- it drops
-- the row from the local dirty set the same way it already does for any
-- other rejection (a CHECK violation, a future-dated push, an
-- unauthorized write). Confirmed by inspection, not assumed: this guard
-- introduces no new shape for the client to learn.
...
comment on function public.sync_push(jsonb, jsonb, jsonb, jsonb, jsonb, jsonb, jsonb) is
  ...
  '20260915200001_sync_push_tombstone_resurrection_guard.sql changed on '
  'top of 20260915160000_sync_push_sole_write_path.sql. Issue #201 '
  ...
  '"no stored row" branch now rejects (does not insert) an incoming id '
  'whose updated_at predates enforce_retention()''s own 180-day day_entries '
  ...
  'row whose tombstone was already hard-deleted. No other behavior change.';
