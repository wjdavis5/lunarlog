-- Migration: 20260909160000_pms_day_marker.sql
--
-- Issue #220 (P1, epic:predictions-insights): PMS as a first-class logged
-- and predicted phase. The issue's Assumptions defer the exact schema
-- shape to the tracking-model issues (#247/#251, still open); following
-- the brief's coordination note, the minimal honest shape landed here is
-- a new synced `day_entries` marker column, on the exact precedent of
-- #159's provenance columns (20260908170000_import_provenance.sql) and
-- #131's/U1's containment-guard pattern:
--
--   1. `day_entries` gains `pms boolean not null default false` -- the
--      marker is deliberately NOT a member of the 17-tag taxonomy (the
--      issue: a day can be PMS without also being tagged for every
--      symptom present, mirroring Clue's own separation of "PMS" from
--      the individual symptoms that might co-occur with it).
--   2. `day_entries_tombstone_pms_check` (CHECK, deleted_at is null or
--      pms = false) -- the structural backstop mirroring
--      day_entries_tombstone_flow_check (issue #224): pms is health
--      content, so a tombstone carries no marker regardless of which
--      code path wrote the row. Safe to add as a validating constraint
--      without a backfill: the column is new, so every existing row
--      (tombstones included) already reads false.
--   3. `grant update (pms) on table public.day_entries to authenticated`
--      -- additive column-list grant (KTD15); the table-level select
--      grant already covers reading it.
--   4. `sync_push`: create-or-replaced from its latest body
--      (20260909141503_care_notes_visit_prep.sql, the current tip of
--      main -- this file sorts after it per Migration Flow item 7),
--      copied verbatim with only the day_entries section touched:
--      `pms` joins the key allowlist, is parsed ahead of the tombstone
--      if/else, cleared in every tombstone-producing site (the direct
--      client soft-delete branch, the same-date resolver's loser
--      tombstoning UPDATE, and the incoming-loses branch), round-tripped
--      on the INSERT path, and written on the UPDATE path behind a
--      `v_row ? 'pms'` containment guard so a pre-#220 client's push
--      never silently clears an already-stored marker (the U1/#131/#159
--      pattern). Same 7-argument signature -- no drop needed, exactly
--      one sync_push afterwards. Same-date collisions: pms is a single
--      boolean on one row (not an array), so it stays last-writer-wins
--      like flow/note; only the tags union applies.
--
-- Client half (same PR): Drift schema v12 (lib/data/db/tables.dart's
-- DayEntries.pms + _upgradeToV12), the domain field, the row codec,
-- storage upsert/tombstone/apply paths, export v7/import round-trip, and
-- the 6-cycle PMS averages plus the predicted band in
-- lib/domain/prediction/pms.dart. Reminder wiring is #178, not here.

-- ---------------------------------------------------------------------------
-- 1. day_entries.pms + the tombstone backstop
-- ---------------------------------------------------------------------------

alter table public.day_entries
  add column pms boolean not null default false;

comment on column public.day_entries.pms is
  'Issue #220: first-class PMS marker -- the day was premenstrual, deliberately outside the tag taxonomy (a day can be PMS without also being tagged for each symptom present). Cleared on a tombstone like every other payload column (day_entries_tombstone_pms_check is the structural backstop); sync_push writes it only when the payload carries the pms key, so a pre-#220 client never clears a stored marker.';

alter table public.day_entries
  add constraint day_entries_tombstone_pms_check
  check (deleted_at is null or pms = false);

comment on constraint day_entries_tombstone_pms_check on public.day_entries is
  'A tombstoned row (deleted_at is not null) always carries pms = false - the structural backstop for issue #220, independent of which code path wrote the row (mirrors day_entries_tombstone_flow_check).';

-- ---------------------------------------------------------------------------
-- 2. Privileges: additive column-list grant (KTD15)
-- ---------------------------------------------------------------------------

grant update (pms) on table public.day_entries to authenticated;

-- ---------------------------------------------------------------------------
-- 3. sync_push: create-or-replaced from its latest body
--    (20260909141503_care_notes_visit_prep.sql), copied verbatim except
--    for the day_entries changes described in this migration's header,
--    item 4. Same 7-argument signature, so CREATE OR REPLACE works in
--    place -- exactly one sync_push afterwards.
-- ---------------------------------------------------------------------------
