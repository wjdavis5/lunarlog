-- Migration: 20260915020000_bulk_import_pms_tombstone.sql
--
-- Issue #140 review follow-up (LLA-060, P2): `bulk_import_entries`
-- (20260908190000_bulk_import.sql, last re-emitted by
-- 20260908200000_flow_model.sql for #247's flow model) predates #220's PMS
-- marker (20260909160000_pms_day_marker.sql) entirely -- `pms` never
-- joined its column list, its SET lists, or its allowlist. `sync_push`
-- itself got the correct treatment when PMS landed: every tombstone-
-- producing branch forces `pms = false` (the structural
-- `day_entries_tombstone_pms_check` backstop, `deleted_at is null or pms =
-- false`), matching `flow`/`tags`/`note`. `bulk_import_entries` was never
-- touched, so BOTH of its write paths can tombstone an existing row
-- WITHOUT clearing a stored `pms = true`:
--
--   * `updated`'s `UPDATE ... SET deleted_at = u.parsed_deleted_at` (the
--     by-id path) leaves `d.pms` completely untouched -- an existing live
--     row with `pms = true` that a bulk chunk tombstones by id keeps
--     `pms = true` on the now-deleted_at-set row.
--   * `upserted`'s `on conflict (profile_id, source, source_id) do update`
--     (the source-conflict path) has no `pms` in its SET list at all,
--     so the SAME thing happens when a chunk's conflict arbiter finds and
--     tombstones an existing row.
--
-- Either way, the `UPDATE` statement itself then violates
-- `day_entries_tombstone_pms_check` -- a `check_violation`, which this
-- function's own `exception when unique_violation` handler does not catch
-- -- so it propagates up and aborts the WHOLE CHUNK (every other row in the
-- same `p_rows` call, not just the one row with the stale `pms`), the
-- exact per-row-not-per-chunk failure this migration's own header says
-- `bulk_import_entries` exists to prevent.
--
-- Fix: both write paths now clear `pms` to `false` whenever the row is
-- BECOMING a tombstone (mirroring `flow`'s `'none'` treatment there
-- already), and otherwise leave the stored value alone -- `pms` is not a
-- bulk-import-carried field (Clue and similar bulk sources have no PMS
-- concept to import; it stays outside `c_row_keys`, unchanged), so there is
-- never an incoming value to apply on a live write, only a stored one to
-- either keep or clear. Body carried forward verbatim from
-- 20260908200000_flow_model.sql other than the two `pms = case ...` lines
-- called out inline below and this header/comment.
--
-- Client half: none -- `pms` was already handled correctly end to end on
-- the Dart side (day-entry import/export, `sync_push`); this is a
-- server-only fix to the one RPC that missed it.
