-- Migration: 20260912000000_health_sync_export_marker.sql
--
-- Issue #186 (P1, epic: health-sync) -- the sync-mechanics foundation's
-- server half. Two additive changes:
--
--  1. `public.observations.exported_to_platform_at timestamptz` (nullable):
--     the client-stamped instant a row's content was last written to a
--     health platform (HealthKit / Health Connect), so a round-trip write is
--     detectable when the same row returns through a future import
--     (#217/#228). Column only -- no index, no grant change: observations
--     carries the table-wide `authenticated` grants #240 established, and
--     this provenance marker needs no special RLS treatment (it is the same
--     class of column as `source`/`source_id`/`import_id`).
--
--  2. `sync_push` re-emitted (body carried forward verbatim from
--     20260911000000_sync_push_tags_validation.sql, still the latest
--     definition -- no migration since has re-emitted it) so the new column
--     round-trips: `exported_to_platform_at` joins `c_observation_keys`,
--     is parsed unconditionally (provenance, like `import_id`, survives a
--     tombstone by design), lands in the INSERT column list, and the UPDATE
--     path applies the established `v_row ? 'exported_to_platform_at'`
--     containment guard (the PR #108 review item #3 / #131 old-client
--     pattern) so a pre-#186 client's push that omits the key never clears
--     an already-stored value.
--
-- The per-profile health-sync consent flag (`profile_modes.health_sync_consent`)
-- that issue #186 also requires already exists server-side -- it was added
-- with the profile_modes table in 20260909000000 (Issue #188, D-29), the
-- #153 (HS-1) consent carried into the life-stage-mode table. No grant,
-- policy, trigger, or index changes.
--
-- Filename sorts after 20260911180000_cascade_day_entry_tombstone.sql, the
-- latest migration on main at dispatch time (Migration Flow item 7).

-- ---------------------------------------------------------------------------
-- 1. The observations round-trip-write marker column.
-- ---------------------------------------------------------------------------

alter table public.observations
  add column exported_to_platform_at timestamptz;

-- The column joins the established column-scoped UPDATE grant (observations
-- is UPDATE-granted per-column, so a sync_push UPDATE referencing the new
-- column would otherwise be denied in full — the #240/#159 pattern; import_id
-- got its own line the same way).
grant update (exported_to_platform_at) on table public.observations to authenticated;

-- ---------------------------------------------------------------------------
-- 2. sync_push: exported_to_platform_at joins the observations keys, the
--    parse block, the INSERT, and the UPDATE (behind the `v_row ? 'key'`
--    containment guard). Full function body carried forward verbatim from
--    20260911000000_sync_push_tags_validation.sql other than the additions
--    called out inline below. Same 7-argument signature, so CREATE OR
