-- ===========================================================================
-- 20260920130000_supplies_kit.sql
...
--      definition (20260920110000_profile_irregular_framing.sql), adding
--      `kind` exactly like the other optional-column siblings: defaulted on
--      parse, inserted on the INSERT path, and containment-guarded on the
--      UPDATE path so an older client's push never rewrites a stored kind.
--      The ten DERIVED allowlist constants are re-derived through
...
--      Everything else, including the #853 irregular_framing handling this
--      base carries, is preserved.
--
...
begin
  ...
  -- The 20260920110000 statement verbatim except the ten array
  ...
  -- lines in the visit_prep_items loop). The #853 irregular_framing and
  -- #861 postpartum_birth_date edits it inherited are carried as-is.
  v_body := $ddl$
