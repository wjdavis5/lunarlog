-- Migration: 20260903080000_sync_push_concurrency.sql
-- Fixes:
-- 1. Issue #14: Serialise sync_push per user via pg_advisory_xact_lock to prevent server_version gaps
-- 2. Issue #13: Re-raise transient concurrency errors (40P01, 40001, 55P03) in per-row exception blocks
...
comment on function public.sync_push(jsonb, jsonb) is
  'Batch upsert of profiles then day entries under RLS with per-row rejection, LWW guard, advisory lock serialization, and transient deadlock re-raising.';
