-- Migration: 20260917120000_sync_push_derived_allowlists.sql
--
-- Issue #181 (P2, chore): "generate database.types.ts snapshot in CI;
-- derive sync_push allowlists". The database half of the issue: sync_push's
-- nine per-table key allowlists (c_profile_keys, c_day_entry_keys,
-- c_observation_keys, c_profile_mode_keys, c_cycle_override_keys,
-- c_care_note_keys, c_visit_prep_item_keys, c_merge_event_keys,
-- c_tag_registry_keys) were hand-restated literal arrays, restated by every
-- re-emission. The birth_year/relationship "old client nulls a stored
-- value" defect (PR #108 review item #3, fixed in 20260906160000) is the
-- exact bug class a hand-maintained list produces: adding a synced column
-- required a coordinated edit across the table DDL, the client codec, and
-- this array, and forgetting the array made the column silently
-- un-syncable (every push carrying it rejected as an unknown key).
--
-- Design (the issue allows derivation "at function-definition or call
-- time"; this migration picks definition time -- materialized into the
-- function body -- for two reasons):
--
--   1. A per-call information_schema read sits on the sync hot path
--      (information_schema.columns is a multi-join catalog view, and the
--      DECLARE-block constants re-evaluate on every call).
--   2. sync_push is SECURITY DEFINER (20260915160000). If the accepted-key
--      set were re-derived from mutable catalog state on every call, a
--      dashboard/Studio grant change could silently widen what the RPC
--      accepts, with no code review at all. Materializing at CREATE time
--      pins the accepted set to exactly what this migration's own
--      assertions approved.
--
-- Drift after materialization is caught, not trusted: the pgTAP suite
-- (supabase/tests/sync_push_derived_allowlists_test.sql) re-derives the
-- nine sets from information_schema AT TEST TIME and compares them with
-- the arrays embedded in the live function body, so a later migration
-- that adds (and grants) a synced column without re-emitting sync_push
-- through this derivation FAILS the suite -- the exact regression class
-- the issue cites.
--
-- The derivation model, in three pieces:
--
--   * SOURCE: every column information_schema reports for the table.
--   * IGNORED (per table): server-stamped columns, never accepted from a
--     payload because the RPC stamps them from the caller --
--     day_entries/day_entry_merge_events: created_at;
--     profile_tag_registry: created_by, created_at; visit_prep_items:
--     checked_by_user_id, checked_at. Everything else the schema
--     historically tolerated-but-never-read (user_id, server_version,
--     logged_by_user_id, last_modified_by_user_id, transferred_at,
--     transferred_to_user_id, created_at on profiles/observations/
--     care_notes) is an ordinary real column and flows in through the
--     source untouched.
--   * EXTRAS (per table): observations, care_notes, and visit_prep_items
--     additionally admit the legacy key 'user_id' -- tolerated, never
--     read, and not a column any of these tables has ever had (it rides
--     along in the hand-written arrays this migration replaces, so it is
--     kept to not reject an old client that echoes one).
--
-- What this migration does:
--
--   1. Creates public.sync_push_payload_keys(text) -- a STABLE SQL helper
--      encoding the model above. It is the single source of truth for the
--      derivation: this migration's DO block materializes from it, the
--      pgTAP suite re-derives through it, and future sync_push
--      re-emissions should embed through it again (see AGENTS.md's
--      Migration Flow). EXECUTE is revoked from public, anon, AND
--      authenticated (the enforce_retention pattern -- the local stack
--      auto-grants authenticated execute on public-schema functions, so a
--      public/anon-only revoke would leave it client-callable; the
--      helper is migration/test tooling, not a client API).
--
--   2. A DO block that:
--      a. derives the nine arrays through the helper;
--      b. sanity-asserts each is non-empty and identifier-shaped (every
--         element matches ^[a-z_][a-z0-9_]*$, which is what makes the
--         array-literal embedding below safe by construction);
--      c. asserts -- as the acceptance baseline -- that each derived set
--         reproduces the corresponding hand-written array from
--         20260917000000 (still the latest definition on main at dispatch
--         time) EXACTLY, proving the refactor behavior-preserving at
--         apply time: no key admitted today is dropped, no key rejected
--         today is admitted;
--      d. builds the new CREATE OR REPLACE FUNCTION statement from a
--         template -- the 20260917000000 body verbatim except the nine
--         array declarations, replaced by placeholder tokens -- and
--         substitutes each token with the derived '{a,b,...}' literal;
--      e. executes it, then re-reads pg_proc and asserts the embedded
--         declarations landed in canonical form (no placeholder remains,
--         and the single-line declaration shape the suite extracts is
--         present; all nine declarations are then extracted and compared
--         against a fresh derivation by the pgTAP suite, which runs the
--         same check for every table).
--
--      The signature is unchanged (nine jsonb parameters, the same
--      defaults), so this is a plain CREATE OR REPLACE: no overload drop
--      dance, and CREATE OR REPLACE preserves the existing EXECUTE grants
--      (20260917000000's revoke-from-public/anon +
--      grant-to-authenticated stands).
--
--   3. Nothing else. No table, grant, policy, trigger, or index changes;
--      every other function is untouched. Coverage:
--      supabase/tests/sync_push_derived_allowlists_test.sql.
--
-- The CI snapshot half of issue #181 (supabase/database.types.ts and its
-- freshness step in ci.yml's db-tests job) ships in the same PR but
...
create or replace function public.sync_push_payload_keys(p_table text)
returns text[]
language sql
stable
set search_path = ''
    ...
    array(
      select c.column_name::text
       ...
       where c.table_schema = 'public'
         ...
         -- server-stamped columns: never accepted from a payload (the
         -- RPC stamps them from the caller), so they are removed from
         -- the derived allowlist even though they are ordinary table
         -- columns. (::text because column_name's type is the
         -- sql_identifier domain, and `<> all` against text[] needs
         -- text.)
         and c.column_name::text <> all (
           case p_table
             when 'day_entries'            then array['created_at']
             when 'day_entry_merge_events' then array['created_at']
             when 'profile_tag_registry'   then array['created_by', 'created_at']
             when 'visit_prep_items'       then array['checked_by_user_id', 'checked_at']
             else '{}'::text[]
           end
         )
       order by c.column_name
    )
    -- legacy tolerated keys: accepted but never read, and NOT columns of
    -- these tables (kept so an old client echoing one is not rejected).
    || case p_table
         when 'observations'     then array['user_id']
         when 'care_notes'       then array['user_id']
         when 'visit_prep_items' then array['user_id']
         else '{}'::text[]
       end;
$$;

comment on function public.sync_push_payload_keys(text) is
  'Issue #181: the derived per-table key allowlist for sync_push -- every column of the named synced table (information_schema), minus that table''s server-stamped columns (day_entries/day_entry_merge_events: created_at; profile_tag_registry: created_by, created_at; visit_prep_items: checked_by_user_id, checked_at), plus its legacy tolerated keys (observations/care_notes/visit_prep_items: user_id). Materialized into sync_push''s body at CREATE time by the migration''s DO block; pinned against the live function by supabase/tests/sync_push_derived_allowlists_test.sql. Migration/test tooling, not a client API (EXECUTE revoked from public, anon, and authenticated).';

revoke execute on function public.sync_push_payload_keys(text) from public, anon, authenticated;

-- ---------------------------------------------------------------------------
-- 2. Materialize: derive the nine arrays, prove they reproduce the
--    hand-written sets they replace, and re-emit sync_push with them
--    embedded. The template below is the 20260917000000 statement
--    verbatim except the nine array declarations (placeholder tokens).
-- ---------------------------------------------------------------------------
...
declare
  ...
  v_body text;
begin
  -- (b) + (c): sanity-check every derived set and prove it reproduces the
  -- hand-written 20260917000000 array it replaces, exactly.
  for v_t in select unnest(v_tables) loop
    ...
    -- The hand-written arrays from 20260917000000_profile_tag_registry.sql,
    -- restated once here as the acceptance baseline (set equality; the
    -- arrays are only ever used with `<> all`, so element order carries no
    -- behavior).
    v_prior := case v_t
    ...
    if v_prior is null then
      raise exception 'no prior allowlist baseline restated for %', v_t
        using errcode = 'object_not_in_prerequisite_state';
    end if;

    if not (v_derived @> v_prior and v_prior @> v_derived) then
      raise exception 'derived allowlist for % does not reproduce the 20260917000000 hand-written set (derived: %; prior: %)',
        v_t, v_derived, v_prior
        using errcode = 'object_not_in_prerequisite_state';
    end if;
  end loop;

  -- (d): build the new statement from the verbatim-carried template and
  -- substitute each placeholder with its derived array literal. Elements
  -- are identifier-shaped (asserted above), so '{a,b,...}' needs no
  -- escaping.
  v_body := $ddl$
...
$ddl$;
  ...
  -- (e): execute, then prove the embedded declarations landed in the
  -- canonical single-line form the pgTAP suite regex-extracts.
  execute v_body;
