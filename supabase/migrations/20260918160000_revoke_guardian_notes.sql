-- ===========================================================================
-- 20260918160000_revoke_guardian_notes.sql
-- Issue #867 (P1, fix(sharing)): revoke_guardian never removes the revoked
-- author's guardian notes, contradicting PRIVACY.md; orphaned notes become
-- un-removable.
--
-- Background:
--   PRIVACY.md guarantees that when a guardian who wrote a guardian note is
--   revoked from a profile, their guardian notes are removed. However,
--   revoke_guardian previously did not touch guardian_notes. Once revoked,
--   the ex-guardian can no longer remove their notes, leaving their free-text
--   health notes about a child live for every remaining guardian. Furthermore,
--   sync_push strictly checked logged_by_user_id = auth.uid(), which meant
--   an orphaned note whose author was deleted (logged_by_user_id set to null)
--   could never be removed by anyone.
--
-- Changes:
--   1. Re-emit public.revoke_guardian(p_profile_id text, p_target_user_id uuid):
--      Tombstone all live guardian_notes written by p_target_user_id for
--      p_profile_id (body = '', deleted_at = v_now,
--      updated_at = greatest(updated_at, v_now),
--      last_modified_by_user_id = v_uid).
--   2. Re-emit public.sync_push(...):
--      In the p_guardian_notes loop, allow primary_guardian to tombstone an
--      orphaned note (logged_by_user_id is null and v_deleted_at is not null).
--      Edits to non-tombstone fields by non-authors remain strictly rejected.
-- ===========================================================================

-- ---------------------------------------------------------------------------
-- 1. Re-emit public.revoke_guardian: tombstone the revoked guardian's notes
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- Issue #867: PRIVACY.md guarantees that when a guardian is revoked from
  -- a profile, their guardian notes are removed. Tombstone any active notes
  -- written by the revoked guardian on this profile (clearing body to ''
  -- per guardian_notes_tombstone_payload_check, recording deleted_at,
  -- and updating last_modified_by_user_id to the caller). Triggers on
  -- guardian_notes bump server_version and emit the change notification.
  update public.guardian_notes
     set deleted_at = v_now,
         ...
         last_modified_by_user_id = v_uid
   where profile_id = p_profile_id
     and logged_by_user_id = p_target_user_id
...
comment on function public.revoke_guardian(text, uuid) is
  ...
  'Issue #867: tombstones all live guardian notes authored by the revoked '
  'guardian for this profile, fulfilling PRIVACY.md guarantees.';
...
-- ---------------------------------------------------------------------------
-- 2. Re-emit public.sync_push: allow primary_guardian to tombstone orphaned notes
-- ---------------------------------------------------------------------------
do $derive$
declare
...
comment on function public.sync_push(
  ...
  'Issue #867: allows primary_guardian to tombstone orphaned (authorless) '
  'guardian notes.';
