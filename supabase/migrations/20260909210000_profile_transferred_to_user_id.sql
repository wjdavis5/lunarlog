-- Migration: 20260909210000_profile_transferred_to_user_id.sql
-- Issue #296: the "transferred to whom" signal the health-sync minor gate
-- (issue #153) needs before `AppConfig.healthSyncMinorBindingAllowed` may
-- ever be flipped.
--
-- `profiles.transferred_at` records only THAT ownership moved, so the
-- client guard's `transferredAt != null && isOwner` conjunct was also
-- satisfied by a parent->co-parent transfer with that co-parent signed in
-- - it could not prove "transferred to the minor's own account" the way
-- the policy doc claims. This migration gives the profile row itself a
-- durable record of WHO accepted the last transfer:
--
--   1. `profiles.transferred_to_user_id uuid` - the accepting user's id,
--      stamped by `accept_ownership_transfer` in the same statement that
--      already moves `user_id` and stamps `transferred_at` (so the two
--      can never disagree). Ownership state, exactly like
--      `transferred_at`: NO column grant to `authenticated`, and no
--      `sync_push` write path - tolerated-but-never-read there, so a
--      client that pulls the column back down and echoes it in a later
--      push is not rejected for carrying an "unknown key" (the U1
--      precedent). The FK mirrors `user_id`'s own shape; it is inert in
--      practice, since the transfer target IS the owner - deleting that
--      user cascades the profile away regardless.
--   2. Backfill: rows with `transferred_at` set get
--      `transferred_to_user_id = user_id`. Exact, not approximate:
--      `profiles.user_id` moves ONLY inside `accept_ownership_transfer`
--      (every other `update public.profiles` in this tree names content
--      columns; `sync_push`'s insert path relies on the `auth.uid()`
--      default and its update path never names `user_id`), so the current
--      owner of a transferred profile is exactly who accepted its last
--      transfer. Rows never transferred stay NULL on both columns.
--   3. `create or replace accept_ownership_transfer` - carried forward
--      verbatim from 20260906180000_ownership_transfer_rpcs.sql (still
--      its latest definition) other than the one new assignment and its
--      comment: `transferred_to_user_id = v_uid`.
--   4. `create or replace sync_push` - carried forward verbatim from
--      20260909160000_pms_day_marker.sql (its latest definition; this
--      file sorts after it per Migration Flow item 7) other than the one
--      new tolerated key and the function comment.
--
-- The client half (same PR): the domain `Profile.transferredToUserId`
-- field, the drift column (schema v13), the row codec's pull-only decode,
-- and the tightened guard in `health_sync_binding.dart`, which now
-- requires `transferredAt != null && transferredToUserId ==
-- signedInUserId` for the minor exception. A NULL target (a pre-backfill
-- row, or a row pulled from a not-yet-upgraded server) fails closed.
...
alter table public.profiles
  add column transferred_to_user_id uuid
    references auth.users (id) on delete cascade;

-- Exact per the header: user_id moves only inside
-- accept_ownership_transfer, so the current owner of a transferred
-- profile is exactly who accepted its last transfer.
update public.profiles
   set transferred_to_user_id = user_id
 where transferred_at is not null
   and transferred_to_user_id is null;

comment on column public.profiles.transferred_to_user_id is
  'Issue #296: the account that accepted this profile''s last ownership
   transfer, or null if it never has. Stamped only by
   accept_ownership_transfer, alongside transferred_at. Ownership state -
   not client-writable (no column grant, no sync_push write path); the
   client-side health-sync minor gate requires this to equal the signed-in
   user id before a transferred minor profile may bind.';

-- ---------------------------------------------------------------------------
-- 2. accept_ownership_transfer: stamp the target
-- ---------------------------------------------------------------------------
