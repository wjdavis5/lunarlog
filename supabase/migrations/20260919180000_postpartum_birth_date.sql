-- ===========================================================================
-- 20260919180000_postpartum_birth_date.sql
-- Issue #861: Postpartum mode never asks when the birth was.
--
-- Switching to Postpartum stamps `profile_modes.mode_started_on = today`,
-- so the Today card reads "Day 0 of postpartum" for anyone who does not
-- flip the switch in the delivery room. `lib/domain/postpartum.dart`
-- documents the mode start as a deliberate *surrogate* for the birth date;
-- the wording stays honest ("of postpartum"), the number is not useful.
--
-- Change:
--   1. Add the nullable public.profile_modes.postpartum_birth_date date
--      column -- the optional actual birth date. Null means "not supplied"
--      and keeps the existing mode-start surrogate; supplied means the
--      Postpartum day counter runs from it. Optional by construction (the
--      issue is explicit: do not force a required field into a flow that
--      works today), so no NOT NULL, no default, no CHECK of its own --
--      the client validates entry against the shared day-entry date bounds
--      (future / before the profile's birth year, Issue #848).
--   2. GRANT UPDATE (postpartum_birth_date) to authenticated -- the same
--      column-list grant every sibling profile_modes column carries.
--   3. Re-emit public.sync_push (signature unchanged) from its current
--      definition (20260919170000_profile_tag_registry_sync_conflict.sql),
--      adding `postpartum_birth_date` exactly like its `estimated_due_date`
--      sibling: declared, regex-guarded/parsed, inserted, and
...
--      Migration Flow item 9) -- the profile_modes derivation reads
--      information_schema, so the new column joins c_profile_mode_keys
--      automatically once the ALTER has run; the parse/INSERT/UPDATE
--      statements are the only hand edits. Everything else carries forward
--      unchanged.
--
-- No RLS change: profile_modes policies are row-scoped, not column-scoped.
-- delete_account_data()/export_account_data() are untouched --
-- profile_modes dies with its profile row (ON DELETE CASCADE) and is
-- deliberately never exported server-side (the #240 precedent).
-- ===========================================================================
...
alter table public.profile_modes
  add column postpartum_birth_date date;

comment on column public.profile_modes.postpartum_birth_date is
  'Issue #861 (Postpartum mode): the optional birth date, yyyy-MM-dd. Supplied (or left blank) when the operator selects Postpartum; the day counter runs from it when present and falls back to mode_started_on when null. Synced like every other profile_modes column (sync_push apply-path guarded by the ? containment check, so an older client''s push never clears a stored value). Kept on exit rather than cleared: the mode column says whether the postpartum is current, this stays as the record of the one that was, and any later re-entry overwrites it. No CHECK of its own -- the client validates the value against the shared day-entry date bounds (Issue #848).';

-- ---------------------------------------------------------------------------
-- 2. Privileges (the 20260909000000 section 4 column-list shape)
-- ---------------------------------------------------------------------------

grant update (postpartum_birth_date) on table public.profile_modes to authenticated;
...
begin
  ...
  -- The 20260919170000 statement verbatim except the ten array
  -- declarations (placeholder tokens) and the Issue #861 profile_modes
  -- postpartum_birth_date edits (one new declared variable plus the
  -- parse/insert/update lines in the profile_modes loop).
  v_body := $ddl$
