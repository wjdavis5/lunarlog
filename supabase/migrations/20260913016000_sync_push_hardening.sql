-- Migration: 20260913016000_sync_push_hardening.sql
--
-- Consolidated sync_push re-emission covering the sync_push-body portions
-- of six issues from this PR's batch. sync_push is a single ~900-line
-- function that must be redefined in full on every change (CREATE OR
-- REPLACE takes the whole body); rather than re-emitting that entire body
-- six near-identical times across six separate migrations (one per issue,
-- each differing from the last by a few lines, and each carrying the very
-- real risk of a transcription slip in the ~90% that DIDN'T change), this
-- migration bundles every sync_push-body delta into one pass, built by
-- applying each change as a scripted, asserted text transformation against
-- the prior full body (20260912000000_health_sync_export_marker.sql) so
-- every unrelated line is provably untouched. This is a deliberate,
-- documented deviation from "one migration per concern" for this function
-- specifically - every OTHER part of every issue below (new tables,
-- trigger functions, RPCs, indexes, CHECK constraints) still gets its own
-- dedicated migration. Flagged for reviewer scrutiny per this task's own
-- instructions: anything touching sync_push's accept logic deserves an
-- extra look.
--
-- What changed, by issue:
--
--   Issue #518 (P1, minor gate durability): `is_minor` gets the same
--   `v_row ? 'key'` containment guard on the UPDATE path that
--   birth_year/relationship/mode already have - an old client, or any
--   partial-row writer, that omits the key no longer silently un-marks a
--   minor's profile. (The rest of #518 - the gate in
--   upsert_prediction_projection/get_prediction_projection and the
--   BEFORE UPDATE trigger revoking live prediction_connections - lands in
--   20260913011000_prediction_minor_gate_durable.sql.)
--
--   Issue #521 (P1, pull cursor watermark): `pg_advisory_xact_lock(hashtext(...))`
--   switches to `hashtextextended(..., 0)` - `hashtext()` is `int4` and can
--   collide across different UUIDs; `hashtextextended` returns `int8`,
--   astronomically less likely to. (The rest of #521 - sync_watermark()
--   and the sync_inflight bookkeeping table - lands in
--   20260913015000_sync_watermark.sql, which this migration depends on:
--   set_server_version(), re-emitted there, is what actually populates
--   sync_inflight; this migration only changes the lock call.)
--
--   Issue #524 (P1, tombstone cascade LWW rewind): a live observation may
--   never point at a tombstoned day_entry - closes the half of this issue
--   the cascade trigger's own greatest() guard
--   (20260913012000_cascade_tombstone_lww_guard.sql) cannot close by
--   itself (that fix stops the cascade from rewinding a child's clock
--   backward; this stops a RACING push from landing a live row under an
--   already-tombstoned parent in the first place).
--
--   Issue #562 (P2, day_entries column grants): the day_entries UPDATE no
--   longer assigns `profile_id` in its SET clause (the value was already
--   proven equal by the pre-existing immutability check - this was always
--   a no-op assignment). Required because
--   20260913017000_day_entries_column_grants.sql revokes the `authenticated`
--   UPDATE grant on that column entirely; sync_push (SECURITY INVOKER)
--   would otherwise fail outright the moment that migration lands. That
--   migration is numbered AFTER this one deliberately, so a real
--   `supabase db push` never has a window where the grant is gone but
...
--   of distinct profile_ids across all six arrays. Per this task's
--   explicit instruction, this migration does NOT restructure sync_push to
--   set-based writes - the per-row loop-with-exception-block shape (and
--   its per-row subtransaction cost) is otherwise unchanged; only the
--   total-row ceiling and the role-lookup redundancy are addressed.
--
--   Issue #566 (P2, unbounded client-authored updated_at): every one of
--   the seven loops now rejects (as a per-row rejection, via the existing
--   exception-handling block - never the whole batch) any row whose
--   `updated_at` is more than five minutes ahead of the server's own
--   `now()`. A badly-fast client clock would otherwise win every future
--   LWW race against every other guardian indefinitely, and every decline
--   hands back the stale server copy in `resolved`, which the client
--   applies - so affected guardians would see their own edits silently
--   reverted on their own screens.
--
-- Same 7-argument signature as every version since 20260908160000, so
-- CREATE OR REPLACE works in place with no DROP.
...
comment on function public.sync_push(jsonb, jsonb, jsonb, jsonb, jsonb, jsonb, jsonb) is
  ...
  '20260913016000_sync_push_hardening.sql changed. Issue #518: is_minor on '
  ...
  'more than five minutes ahead of the server''s now().';
