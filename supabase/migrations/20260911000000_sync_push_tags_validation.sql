-- Migration: 20260911000000_sync_push_tags_validation.sql
--
-- Issue #96 (P3, defence in depth): the 20260904020000 migration rewrote
-- sync_push and silently dropped the explicit is_valid_tags_array
-- validation that 20260903170000 (Issue #40) had added to the RPC three
-- days earlier. Enforcement since then happens only via the
-- day_entries_tags_check table CHECK at DML time, so the rejection still
-- occurs -- but the earlier migration's stated contract ("Update sync_push
-- RPC to reject non-string elements in tags") was undone without a trace,
-- and the RPC's validation depth is one layer thinner than its own history
-- says it should be.
--
-- The fix here restores the single-call validation the #40 migration
-- intended, in the day_entries non-tombstone tags branch, immediately after
...
--   end if;
--
-- No behavior change: every input the new RPC check rejects was already
-- rejected by day_entries_tags_check (the validator is exactly the CHECK's
-- predicate, tightened by 20260907010000_tags_element_length_check.sql with
-- the per-element char_length <= 64 bound), and the per-row exception
-- handler re-raises only 40P01/40001/55P03, so a 22023 raise lands in the
-- per-row rejected array exactly like a 23514 CHECK violation did. The
-- tombstone branch (v_tags := '[]') needs no validation; merge_tag_arrays
-- results are already-validated strings capped at 32; observations has no
-- tags column.
--
-- What this migration does NOT change (body carried forward verbatim from
-- 20260910171042_sync_push_lock_order.sql, still the latest sync_push
-- definition - no migration since has re-emitted it): the 7-argument
-- signature (CREATE OR REPLACE in place), every other parse/validate/
-- resolve branch, RLS (security invoker, no policy touched), and the
-- authenticated-only grant (revoke/grant re-issued verbatim below).
-- reconcile_realtime_publication() is untouched.
--
-- Filename sorts after 20260910171042_sync_push_lock_order.sql, the latest
