-- ===========================================================================
-- 20260921130000_day_entry_note_private.sql
-- Issue #849 (re-scoped): the one privacy control -- a per-note "private"
-- flag on the subject's own day note.
--
-- The owner's 2026-09-20 re-scope (issue #849): guardians see everything the
-- subject logs (flow, tags, observations, care notes); the single exception
-- is a day note the subject marks private. Privacy is chosen when the note is
-- written -- a note already shared (stored non-empty) cannot be made private
-- retroactively. A note may be inserted private, or flipped false -> true
-- while it is still empty (the note being written now); true -> false is
-- never allowed.
--
-- What this migration owns:
--   1. public.day_entries.note_private boolean not null default false, plus
--      its authenticated UPDATE column grant.
--   2. public.enforce_day_entry_note_private(), a BEFORE INSERT OR UPDATE
--      trigger enforcing the transition rule above (INSERT free; UPDATE:
--      false -> true only while the stored note is null/empty, true -> false
--      never).
--   3. is_profile_subject()/profile_has_subject()/mask_day_entry_note()
--      helpers: the subject (the person the profile is about, #802's
--      is_subject marker) reads a private note in full; every other accepted
--      guardian membership -- any role -- gets note = NULL. Masking is
--      applied in the read path guardians actually use (sync_pull, the only
--      way any client reads day_entries), in sync_push's declined
--      server-copy handback, and in export_account_data(). RLS alone cannot
--      transform a column, so these RPCs are the enforceable shape.
--   4. sync_push re-emitted (signature unchanged) through the #181 derived
--      allowlist DO block: note_private joins c_day_entry_keys from
--      information_schema automatically. The parse/INSERT/UPDATE statements
--      and the resolved-row masking are the hand edits.
--   5. sync_pull re-emitted with the day_entries page masked.
--   6. export_account_data() re-emitted with note_private (and a masked note
--      for a non-subject caller).
--
-- A guardian device holding a masked row (note = NULL, note_private = true)
-- must never clobber the subject's stored text: sync_push's UPDATE path
-- preserves the stored note when the stored row is private and the caller is
-- not its subject, and the client ORs the flag on local writes.
--
-- The server resolver's merge-disclosure text is redacted to '' when the
-- losing note was private; a client-emitted p_merge_events row is not
-- re-validated against privacy (see the PR's "Not done").
-- ===========================================================================
...
comment on column public.day_entries.note_private is
  'Issue #849: this day note is private to the profile subject. Chosen when the note is written (true on INSERT, or false -> true while the stored note is still null/empty); it can never be set true on a note that was already shared, and never cleared back to false. Read in full only by the subject (#802 is_subject); every other accepted guardian membership receives note = NULL from sync_pull/sync_push and a masked note from export_account_data(). A BEFORE trigger (enforce_day_entry_note_private) enforces the transition rule; the sync_push UPDATE path preserves the stored note when a non-subject caller holds a masked copy.';

grant update (note_private) on table public.day_entries to authenticated;

-- ---------------------------------------------------------------------------
-- 2. Subject helpers (the #802 is_subject marker is the identity)
-- ---------------------------------------------------------------------------

create or replace function public.is_profile_subject(p_profile_id text, p_user_id uuid)
returns boolean
language sql
security definer
stable
  ...
  select exists (
    select 1
     ...
     where profile_id = p_profile_id
       ...
       and status = 'accepted'
       and is_subject is true
  );
$$;

comment on function public.is_profile_subject(text, uuid) is
  'Issue #849: true when p_user_id is the accepted subject membership of p_profile_id (#802). The one viewer a private day note is not masked from. Migration/RPC tooling: EXECUTE revoked from public, anon, and authenticated.';

create or replace function public.profile_has_subject(p_profile_id text)
returns boolean
language sql
security definer
stable
...
comment on function public.profile_has_subject(text) is
  'Issue #849: true when p_profile_id has any accepted subject membership. A profile with no designated subject has no one to protect a private note from, so masking is skipped (fail open to the owner). Migration/RPC tooling: EXECUTE revoked from public, anon, and authenticated.';

create or replace function public.mask_day_entry_note(p_row jsonb, p_viewer uuid)
returns jsonb
language sql
security definer
stable
...
as $$
  select case
    when (p_row ->> 'note_private')::boolean
         and public.profile_has_subject(p_row ->> 'profile_id')
         and not public.is_profile_subject(p_row ->> 'profile_id', p_viewer)
      then jsonb_set(p_row, '{note}', 'null'::jsonb)
    else p_row
  end;
$$;

comment on function public.mask_day_entry_note(jsonb, uuid) is
  'Issue #849: returns a day_entries row with note forced to NULL when it is private and p_viewer is a non-subject accepted guardian of a profile that has a designated subject. The one masking point sync_pull, sync_push''s resolved handback, and export_account_data() all funnel through. Migration/RPC tooling: EXECUTE revoked from public, anon, and authenticated.';

revoke execute on function public.is_profile_subject(text, uuid) from public, anon, authenticated;
revoke execute on function public.profile_has_subject(text) from public, anon, authenticated;
revoke execute on function public.mask_day_entry_note(jsonb, uuid) from public, anon, authenticated;

-- ---------------------------------------------------------------------------
-- 3. The transition guard
-- ---------------------------------------------------------------------------

create or replace function public.enforce_day_entry_note_private()
returns trigger
language plpgsql
set search_path = ''
as $$
begin
  if tg_op = 'INSERT' then
    -- A note may be born private (the subject is writing it now).
    return new;
  end if;

  if new.note_private and not old.note_private then
    -- false -> true is allowed only while the stored note is still empty:
    -- the note is being written now, not retroactively hidden.
    if old.note is not null and btrim(old.note) <> '' then
      raise exception 'note_private cannot be set on a note that was already shared'
        using errcode = 'check_violation';
    end if;
  end if;

  if old.note_private and not new.note_private then
    raise exception 'note_private cannot be cleared once set'
      using errcode = 'check_violation';
  end if;
...
comment on function public.enforce_day_entry_note_private() is
  'Issue #849: BEFORE INSERT OR UPDATE guard on day_entries. INSERT is free (a note may be born private); UPDATE allows note_private to stay as it was or flip false -> true only while the stored note is null/empty, and never true -> false. Raises check_violation (23514), which sync_push''s per-row handler turns into a rejected row.';

create trigger day_entries_note_private_guard
  before insert or update on public.day_entries
  for each row execute function public.enforce_day_entry_note_private();

revoke execute on function public.enforce_day_entry_note_private() from public, anon;

-- ---------------------------------------------------------------------------
-- 4. sync_push re-emitted through the #181 derived-allowlist recipe
-- ---------------------------------------------------------------------------
do $derive$
declare
...
begin
  ...
  -- 20260917120000's DO block ran). The day_entries derivation now includes
  -- note_private because the ALTER above ran first; the pgTAP suite's
  -- re-derivation against the live body is the drift catch.
  ...
  -- The 20260920130000 statement verbatim except the ten array declarations
  -- (placeholder tokens) and the Issue #849 note_private edits: two new
  -- declared variables (v_note_private, v_note_hidden), the parse line, the
  -- new column on both day_entries INSERTs, the UPDATE's note/note_private
  -- assignments, the mask_day_entry_note wrapper on both resolved-row
  -- handbacks, and the two merge-discard redactions. Every #851/#853/#861
  -- edit it inherited is carried as-is.
  v_body := $ddl$
...
declare
  ...
  v_pms boolean;
  -- Issue #849: the per-note privacy flag parsed from the payload (default
  -- false), and -- computed just before the store -- whether the STORED
  -- row's private note is hidden from this caller (a non-subject accepted
  -- guardian). When hidden, the incoming note is never written back over
  -- the stored text: a guardian's device holds note = NULL (masked by
  -- sync_pull) and must not clobber the subject's private note.
  v_note_private boolean;
  v_note_hidden boolean;

  v_stored_profile public.profiles%rowtype;
  -- Issue #848: the subject's birth year, looked up per day-entry row so
  -- a live entry dated before the profile was born is rejected into
  -- `rejected` (null when the profile has no birth_year).
  v_entry_profile_birth_year smallint;
  ...
  -- Issue #861: the optional Postpartum birth date.
  v_postpartum_birth_date date;
  ...
  v_other_registry_row public.profile_tag_registry%rowtype;
  v_tag_incoming_wins boolean;
...
begin
  ...
  for v_row in select value from jsonb_array_elements(p_day_entries) order by value ->> 'id' loop
    ...
    begin
      ...
      -- Issue #849: the per-note privacy flag, parsed like pms. An absent
      -- key parses to false so the INSERT path always has a value; the
      -- UPDATE path guards with v_row ? 'note_private' so an old client
      -- omitting the key never clears an already-stored private flag.
      v_note_private := coalesce((v_row ->> 'note_private')::boolean, false);
      v_updated_at := (v_row ->> 'updated_at')::timestamptz;
      if v_updated_at is null then
        raise exception 'updated_at is required';
      end if;
      ...
      if v_deleted_at is not null then
        ...
        -- Issue #848: a live day entry may not be dated more than one
        -- day after the server's own calendar (the +1 tolerates a
        -- client whose local calendar is legitimately a day ahead),
        -- nor before its subject's birth year when one is known. A
        -- row failing either is rejected into `rejected` by this
        -- loop's per-row exception handler, exactly like a validation
        -- or CHECK failure. Deliberately live-only: tombstoning an
        -- already-stored bad row must stay possible (retro-cleaning
        -- is out of scope, and blocking the delete would only strand
        -- the bad row).
        if v_local_date > current_date + 1 then
          raise exception 'local_date is more than one day in the future'
            using errcode = 'invalid_parameter_value';
        end if;
        select p.birth_year into v_entry_profile_birth_year
          from public.profiles p
         where p.id = v_profile_id;
        if v_entry_profile_birth_year is not null
           and extract(year from v_local_date)::int < v_entry_profile_birth_year then
          raise exception 'local_date is before the profile birth year'
            using errcode = 'invalid_parameter_value';
        end if;
      ...
      if found then
        ...
        if not v_accept then
          if not (v_updated_at = v_stored.updated_at
            ...
            -- declined (older, or equal-and-live): hand back the server copy
            v_resolved := v_resolved
              || (public.mask_day_entry_note(to_jsonb(v_stored), v_uid) || jsonb_build_object('table', 'day_entries'));
          end if;
          continue;
        end if;
      else
          ...
          if v_incoming_wins then
            ...
            if v_other.note is distinct from v_note then
              perform public.record_day_entry_merge_discard(
                ...
                -- Issue #849: a private losing note's text is never
                -- disclosed; the discard is still recorded, with empty
                -- retained text.
                case when v_other.note_private then '' else v_other.note end,
            ...
            if v_note is distinct from v_other.note then
              perform public.record_day_entry_merge_discard(
                ...
                -- Issue #849: same redaction for a private incoming loser.
                case when v_note_private then '' else v_note end,
                v_uid,
                coalesce(v_other.last_modified_by_user_id, v_other.logged_by_user_id));
              -- Issue #170: the same discard's content-free feed row --
      ...
      -- Issue #849: a stored private note is hidden from a non-subject
      -- caller, whose own copy carries note = NULL; the UPDATE below must
      -- preserve the stored text rather than write that NULL back.
      v_note_hidden := coalesce(v_stored.note_private, false)
        and not public.is_profile_subject(v_stored.profile_id, v_uid);
      ...
      if v_stored.id is null then
        ...
        update public.day_entries
           set local_date = v_local_date,
               ...
               flow = v_flow,
               tags = v_tags,
               -- Issue #849: a tombstone still clears the note (the
               -- structural CHECK requires it), but a non-subject caller
               -- never overwrites the stored private text with its own
               -- masked NULL; note_private is preserved in both cases (the
               -- guard trigger forbids true -> false).
               note = case
                        when v_deleted_at is not null then null
                        when v_note_hidden then v_stored.note
                        else v_note
                      end,
               note_private = case
                        when v_note_hidden then v_stored.note_private
                        when v_deleted_at is not null then v_stored.note_private
                        when v_row ? 'note_private' then v_note_private
                        else v_stored.note_private
                      end,
  ...
  for v_row in select value from jsonb_array_elements(p_profile_modes) order by value ->> 'profile_id' loop
    begin
      ...
      -- Issue #861: the Postpartum birth date, same shape/guard as its
      -- estimated_due_date sibling.
      if (v_row ->> 'postpartum_birth_date') is not null
         and (v_row ->> 'postpartum_birth_date') !~ '^\d{4}-\d{2}-\d{2}$' then
        raise exception 'postpartum_birth_date is not an ISO calendar date';
      end if;
      v_postpartum_birth_date := (v_row ->> 'postpartum_birth_date')::date;
      ...
      if not found then
        ...
        update public.profile_modes
           set mode = case when v_row ? 'mode' then v_mode else v_stored_mode.mode end,
               ...
               postpartum_birth_date = case when v_row ? 'postpartum_birth_date' then v_postpartum_birth_date else v_stored_mode.postpartum_birth_date end,
  ...
  for v_row in select value from jsonb_array_elements(p_tag_registry) order by value ->> 'id' loop
    v_stored_registry_row := null;
    v_other_registry_row := null;
    v_tag_incoming_wins := null;
    begin
      if jsonb_typeof(v_row) <> 'object' then
        raise exception 'row is not an object';
      end if;
      ...
      -- Issue #825: deterministic same-code collision resolution on
      -- (profile_id, lower(code)) for live writes (the child-table analogue
      -- of observations same-date (category, code) collision dedup).
      -- When two devices concurrently create a custom tag with the same code
      -- under different ULIDs, LWW (tiebreaker: lower ULID id collation)
      -- decides the winner:
      --   - If incoming wins: the previously-stored sibling is tombstoned
      --     and returned in v_resolved; incoming proceeds to land as live.
      --   - If incoming loses: incoming is stored as a payload-free tombstone
      --     at the winner's timestamp and returned in v_resolved (along with
      --     the winning row); 0 rejections occur, ending the push rejection loop.
      if v_deleted_at is null then
        select * into v_other_registry_row
          from public.profile_tag_registry
         where profile_id = v_profile_id
           and lower(code) = lower(v_registry_code)
           and deleted_at is null
           and id <> v_id
         order by id
         for update;
        ...
        if found then
          v_tag_incoming_wins := v_updated_at > v_other_registry_row.updated_at
            or (v_updated_at = v_other_registry_row.updated_at
                and (v_id collate "C") < (v_other_registry_row.id collate "C"));
          if v_tag_incoming_wins then
            -- Incoming row survives; sibling becomes a payload-free tombstone.
            update public.profile_tag_registry
               set deleted_at = v_updated_at,
                   ...
                   hidden_at = null,
                   sort_order = null
             where id = v_other_registry_row.id
             returning * into v_other_registry_row;
            v_resolved := v_resolved
              || (to_jsonb(v_other_registry_row) || jsonb_build_object('table', 'profile_tag_registry'));
          else
            -- Incoming row loses: it is stored as a payload-free tombstone
            -- at the winner's timestamp.
            v_deleted_at := v_other_registry_row.updated_at;
            v_updated_at := v_other_registry_row.updated_at;
            v_registry_display_name := '';
            v_registry_category := '';
            v_registry_intensity_enabled := false;
            v_registry_hidden_at := null;
      ...
      if v_stored_registry_row.id is null then
        ...
        returning * into v_stored_registry_row;
      else
        update public.profile_tag_registry
      ...
      if v_tag_incoming_wins is false then
        v_resolved := v_resolved
          || (to_jsonb(v_stored_registry_row) || jsonb_build_object('table', 'profile_tag_registry'));
      end if;
    exception
  ...
  for v_row in select value from jsonb_array_elements(p_guardian_notes) order by value ->> 'id' loop
    begin
      ...
      if found then
        ...
        -- Issue #867: if the author's account was deleted (logged_by_user_id is null),
        -- allow primary_guardian to tombstone the orphaned note so it does not
        -- remain permanently locked/un-removable. Edits to body/date/tz by anyone
        -- other than the author remain strictly forbidden.
        if v_stored_guardian_note.logged_by_user_id is distinct from v_uid then
          if v_stored_guardian_note.logged_by_user_id is null
             and v_deleted_at is not null
             and v_caller_role = 'primary_guardian' then
            null;
          else
...
-- ---------------------------------------------------------------------------
-- 5. sync_pull re-emitted with the masked day_entries page
-- ---------------------------------------------------------------------------
...
begin
  ...
  v_result := v_result || jsonb_build_object('day_entries', (
    -- Issue #849: a private note is masked to NULL for every non-subject
    -- accepted guardian; the subject receives it in full.
    select coalesce(jsonb_agg(public.mask_day_entry_note(to_jsonb(t), v_uid)), '[]'::jsonb) from (
...
-- ---------------------------------------------------------------------------
-- 6. export_account_data() re-emitted with note_private + masking
-- ---------------------------------------------------------------------------
        ...
        'day_entries', coalesce((
          select jsonb_agg(
            jsonb_build_object(
              ...
              'flow', de.flow,
              'tags', de.tags,
              -- Issue #849: a private note is never disclosed to a
              -- non-subject caller, not even in the owner's export of a
              -- shared/minor profile; the flag itself still rides.
              'note', case
                when de.note_private
                     and public.profile_has_subject(de.profile_id)
                     and not public.is_profile_subject(de.profile_id, v_uid)
                  then null
                else de.note
              end,
              'note_private', de.note_private,
