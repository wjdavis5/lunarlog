-- ===========================================================================
-- 20260919160000_day_entry_date_bounds.sql
-- Issue #848: nothing below the calendar cell rejects a future-dated or
-- pre-birth day entry.
--
-- Background:
--   The only future-date block in the app lived on the calendar cell itself
--   (selectable = !isFuture in month_calendar.dart). Every write path that
--   bypasses the calendar - restore/import, the bulk import seam, the
--   health import, the quick-log FAB with a tampered clock, and this RPC -
--   accepted 2999-01-01, and nothing anywhere compared an entry's
--   local_date against the profile's birth_year. The client side of #848
--   adds DayEntryPolicy.validateDate and calls it from upsertDayEntry,
--   bulkUpsertDayEntries, and the account import; this migration adds the
--   authoritative server-side half so a tampered or older client cannot
--   write an out-of-bounds date at all.
--
-- Change:
--   Re-emit public.sync_push (signature unchanged) with two new checks in
--   the day_entries loop's live branch:
--     1. v_local_date > current_date + 1 is rejected. The +1 tolerance is
--        deliberate: a user whose local calendar is a day ahead of the
--        server's UTC date is still logging a real "today".
--     2. when the row's profile has a birth_year, a local_date whose year
--        precedes it is rejected. Only the year is compared - birth_year
--        carries no month/day.
--   A row failing either raises inside the loop's existing per-row
--   exception handler, so it lands in the same opaque `rejected` array
--   every other per-row failure already uses. Tombstone rows
--   (deleted_at is not null) are exempt: a delete is not new data, and
--   blocking it would strand an already-stored bad row with no way to
--   remove it. Backfilling existing bad rows is out of scope (#848).
--
-- Issue #181 (AGENTS.md Migration Flow item 9): sync_push's per-table key
-- allowlists are DERIVED from public.sync_push_payload_keys(text), not
-- hand-restated. This migration copies 20260918160000's DO-block pattern
-- verbatim - the ten `__SYNC_PUSH_*_KEYS__` placeholders replaced at CREATE
-- time by the derived '{a,b,...}' literals, with the migration-time
-- sanity checks and the post-execute canonical-form assertion carried
-- along - and carries the then-current function body verbatim except for
-- the two #848 edits above. Hand-restating an allowlist here would fail
-- supabase/tests/sync_push_derived_allowlists_test.sql, and should.
--
-- No table, grant, policy, trigger, or index changes. CREATE OR REPLACE
-- preserves the existing EXECUTE grants and function comment (the
-- signature is unchanged). Coverage:
