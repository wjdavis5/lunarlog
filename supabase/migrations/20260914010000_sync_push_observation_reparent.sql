...
-- 20260913016000_sync_push_hardening.sql body (the current definition on
-- main) plus exactly one change: the same-date day-entry resolver now
-- REPARENTS a losing day entry's live observations onto the surviving entry
-- before the loser is tombstoned, instead of letting the day-entry tombstone
-- cascade (`cascade_day_entry_tombstone_to_observations`) wipe them.
--
-- Background: when two offline entries share (profile_id, local_date), the
-- resolver tombstones the LWW loser. That tombstone UPDATE fires the
-- day_entries cascade, which clears every live observation attached to the
-- loser -- so the losing day's structured severity/measurements (the reason
-- observations exists) were destroyed outright, even though the loser's
-- *tags* survive via merge_tag_arrays. The winner's own observations were
-- untouched, but the loser's were not.
--
-- Fix, both winner directions:
--   * incoming-wins: reparent v_other's live observations to v_id BEFORE the
--     statement that tombstones v_other. The cascade then finds no rows with
--     day_entry_id = v_other.id and wipes nothing.
--   * incoming-loses: v_id becomes the tombstone via the insert/update
--     below; if it was an already-stored live row (v_stored.id not null),
--     reparent its live observations to the surviving v_other first.
--
-- Collision handling: a reparented row whose (category, code) already exists
-- LIVE on the surviving entry is deliberately left behind (its `o.code is
-- null or not exists(...)` guard) so the reparent can never violate the
-- `observations_live_profile_date_category_code_uq` partial unique index
-- (20260913018000). Such a row is a genuine same-day duplicate and the index
-- already exists to reject/dedupe it; this fix's job is preserving the
-- non-conflicting children, not arbitrating a head-to-head the index owns.
-- code-null rows are never covered by that partial index and always
-- reparent. last_modified_by_user_id is stamped to the caller because
-- enforce_observation_attribution requires it on every update while
-- auth.uid() is set (same stamping the loser-tombstone statements already
-- do).
--
-- Coverage: supabase/tests/sync_push_observation_reparent_test.sql (new) --
-- both winner directions with disjoint (category, code), plus the reparent
-- leaving a colliding row untouched.
--
-- One schema change, required for the reparent to be possible at all:
--
-- The reparent runs in the incoming-wins branch, before the surviving winner
-- row is persisted (the day_entries loop stores the winner at the end), and
-- the day_entries_live_profile_date_uq partial unique index forbids
-- inserting the winner while the loser is still live -- so the winner can
-- never exist as a LIVE row at reparent time. Rather than defer the
-- observations.day_entry_id FK (which breaks any `ALTER TABLE observations`
-- in a transaction that already wrote observations -- flow_model_test does
-- exactly this, and deferral is transaction-global), the wins branch
-- inserts the winner as a TOMBSTONE first: a tombstone satisfies the FK and
-- is invisible to the live-per-date unique index, the loser's observations
-- reparent to it, the loser is then tombstoned, and the end-of-loop store
-- (which already re-runs for every row) flips the winner from tombstone to
-- live. The FK stays non-deferrable, so all other write paths keep
-- immediate enforcement. The incoming-loses reparent needs no such step:
-- its target (v_other, the surviving row) already exists.
--
-- Otherwise only the sync_push body is re-emitted (CREATE OR REPLACE, same
-- 7-argument signature) from the 20260913016000 body plus the two reparent
-- statements. No table, policy, grant, or FK change.
