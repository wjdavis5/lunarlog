-- ===========================================================================
-- 20260918100000_pregnancy_due_date.sql
-- Issue #192 (feat(modes): Pregnancy mode): adds
-- public.profile_modes.estimated_due_date -- the synced estimated due
-- date a pregnancy entry collects or derives (last recorded period
-- start + 280 days, Naegele's rule; a manual pick when that start is
-- unknown/imported), which the Pregnancy-mode week counter is computed
-- from on every guardian's device. A client-local-only due date was
-- explicitly rejected by the issue: it must sync.
--
-- Follows the 20260906160000_profile_subject_metadata.sql /
-- 20260909120000_provisional_cycle_facts.sql pattern for a new synced
-- profile-scoped column, in three steps:
--
-- 1. ALTER TABLE ... add column estimated_due_date date (nullable: no
--    pregnancy, or one whose due date was never recorded). No CHECK of
--    its own -- the value is a plain future-ish civil date; the client
--    derives it from the last recorded period start (+280d) or collects
--    a manual pick, and a deliberately honest-wide column never rejects
--    a legitimate answer (the 20260909120000 no-future-date-check
--    lesson: the server's current_date is UTC and would reject a
--    legitimate 'today' pick from a zone ahead of UTC).
-- 2. GRANT UPDATE (estimated_due_date) to authenticated -- the
--    column-list grant shape every sibling profile_modes column carries
--    (20260909000000 section 4); sync_push remains the sole write path
--    in practice, but the grant keeps parity with the table's own
--    posture and the #181 derived-allowlist test's
--    granted-outside-allowlist check (every write-granted column must
--    be an allowlisted payload key -- it is, see step 3).
-- 3. sync_push is re-emitted from its current definition
--    (20260918000000_day_entry_history.sql, unchanged since) with
--    exactly the Issue #192 change: the profile_modes branch declares,
--    regex-guards/casts, inserts, and containment-guard-updates
--    `estimated_due_date` exactly like its `mode_started_on` sibling.
--    The nine DERIVED allowlist constants are re-derived through
...
--    profile_modes derivation reads information_schema, so the new
--    column joins c_profile_mode_keys automatically once the ALTER has
--    run; the parse/INSERT/UPDATE statements are the only hand edits.
--    Everything else carries forward unchanged; the signature is
--    unchanged (nine jsonb parameters), so this is a plain CREATE OR
--    REPLACE and the existing EXECUTE grants stand.
--
-- No RLS change: profile_modes policies are row-scoped, not
-- column-scoped; no new policy can be needed for a new column on an
-- already-RLS'd table. delete_account_data()/export_account_data() are
-- untouched -- profile_modes dies with its profile row (ON DELETE
-- CASCADE) and is deliberately never exported server-side (the #240
-- observations precedent).
-- ===========================================================================
...
alter table public.profile_modes
  add column estimated_due_date date;

comment on column public.profile_modes.estimated_due_date is
  'Issue #192 (Pregnancy mode): estimated due date, yyyy-MM-dd. Derived on entry from the last recorded period start + 280 days (Naegele''s rule) or manually supplied when that start is unknown/imported; nullable when no due date was recorded. Synced like every other profile_modes column (sync_push apply-path guarded by the ? containment check, so an older client''s push never clears a stored value). Kept on exit rather than cleared: the mode column says whether a pregnancy is current, this stays as the record of the one that was, and any later re-entry overwrites it.';

-- ---------------------------------------------------------------------------
-- 2. Privileges (the 20260909000000 section 4 column-list shape)
-- ---------------------------------------------------------------------------

grant update (estimated_due_date) on table public.profile_modes to authenticated;

-- ---------------------------------------------------------------------------
-- 3. sync_push: re-emitted from 20260918000000_day_entry_history.sql with
--    the estimated_due_date edits. The DO block below carries that
--    statement's template verbatim except the named Issue #192 edits;
--    allowlist constants are re-derived and substituted per the #181
--    recipe (which is what makes the new column join the payload key set
