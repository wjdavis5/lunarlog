-- Migration: 20260918000000_day_entry_history.sql
--
-- Issue #170 (P2, epic: backend-data): "add day_entry_history table for
-- change-history feed". day_entries carries only logged_by_user_id and
-- last_modified_by_user_id -- the LATEST attribution, not a revision log.
-- A chain of three edits by three guardians is indistinguishable from one
-- edit, and #124's shared activity feed ("which guardian changed what")
-- has nothing to read. This migration adds the audit substrate:
--
--   1. `public.day_entry_history`: one machine-written row per day_entries
--      change -- WHO changed it, WHEN, WHICH column names changed, and the
--      kind of change. CONTENT-FREE BY CONSTRUCTION: the only payload-ish
--      column, `changed_fields`, is a text[] of day_entries COLUMN NAMES
--      enforced against a closed allowlist by a table CHECK
--      (`day_entry_history_changed_fields_check` via
--      `public.is_valid_changed_fields`, the is_valid_tags_array/#94
--      precedent) -- no value from `flow`/`tags`/`note` can ever ride in
--      it, so the table widens no exposure beyond what day_entries itself
--      already grants, and every accepted guardian may read it.
--
--   2. Writes are TRIGGER-ONLY (the notification_outbox posture): an AFTER
--      INSERT OR UPDATE trigger on day_entries
--      (`day_entries_record_history` -> `record_day_entry_change()`)
--      writes exactly one row per change; there is NO client write grant,
--      NO insert/update/delete policy, and NO sync_push parameter. The
--      only other writer is `record_day_entry_merge_history()`, called
--      from sync_push's same-date resolver for the `merged_discard` kind.
--
--   3. changed_by attribution: the trigger reads the acting user off the
--      row sync_push just stamped -- `coalesce(new.last_modified_by_user_id,
--      new.logged_by_user_id, new.user_id)`. sync_push stamps
--      last_modified_by_user_id from the caller's auth.uid() on EVERY
--      insert and update (enforce_day_entry_attribution requires it equal
--      auth.uid() on updates while a session is set), so the pusher IS the
--      changed_by for every ordinary write; bulk_import_entries stamps
--      from the job's created_by; tombstone_profile_content stamps
--      coalesce(v_uid, existing). A row whose attribution is entirely null
--      (only a hypothetical raw service write -- no real path produces
--      one) records NO history row rather than aborting the entry write:
--      an audit emission must never fail the change it audits (the
--      record_day_entry_merge_discard precedent).
--
--   4. Change kinds and the born-dead carve-out:
--      * INSERT live -> 'logged', changed_fields = 'local_date' plus every
--        content column carrying a non-default value (flow <> 'none',
--        tags <> [], note not null, pms, tz <> 'UTC').
--      * UPDATE live->live -> 'updated', changed_fields = the tracked
--        columns that actually differ (local_date, tz, flow, tags, note,
--        pms, source, source_id, import_id, deleted_at). An update that
--        touches nothing tracked (attribution/stamp-only re-saves: the
--        ownership-transfer and delete-account re-homes, a no-change tags
--        union) writes NO row -- there is no change to disclose.
--      * UPDATE live->dead -> 'tombstoned', changed_fields = 'deleted_at'
--        plus the payload columns the tombstone actually cleared.
--      * INSERT already-dead -> NO row. A row that is born dead never
--        exposes a change to any guardian. The two real writers of such a
--        row: the resolver's FK-placeholder insert (a tombstone flipped to
--        live later in the SAME transaction -- recording a phantom
--        "deleted" for the merge WINNER would be actively wrong), and an
--        offline create-then-delete whose tombstone itself arrives via the
--        ordinary pull (the tombstone row is the record; no live content
--        ever existed server-side to disclose a change TO).
--      * merged_discard -> written by the resolver (below).
--      A hard DELETE of a day_entries row records nothing and cannot: the
--      entry_id FK's own ON DELETE CASCADE would remove the new history
--      row in the same statement. Hard deletes only happen on content
--      wipes (profile cascade, tombstone retention purge at 180 days --
--      past this table's own 90-day window) where the wipe is the point.
--
--   5. Interplay with day_entry_merge_events (#130) -- TWO tables, ONE
--      discard, DIFFERENT purposes, both written by the same resolver
--      branches:
--        * day_entry_merge_events carries the RECOVERABLE DISCARDED TEXT
--          (losing_value_text, health content, 30-day retention) for the
--          notice UI -- the losing author gets their writing back.
--        * day_entry_history carries only the FIELD NAME for the feed --
--          content-free, 90-day retention -- so every guardian can see
--          THAT a merge discarded a note on an entry and WHO pushed the
--          winning row, without a second store recording WHAT the note
--          said.
--      A discard that loses text writes both; a flow-only discard writes
--      both; the tables are never merged because their retention windows,
--      their RLS posture equals, and their payloads differ on exactly the
--      axis (content vs. no content) this schema is built around.
--
--   6. Retention: 90 days -- ALREADY WIRED. 20260915200000_nightly_
--      retention_job.sql's enforce_retention() carried a to_regclass-
--      guarded `day_entry_history` purge step (changed_at < now() - 90
--      days) from the day it landed, explicitly waiting for this table to
--      exist ("the moment #170's migration creates that table, this job
--      starts enforcing its 90-day window automatically"). This migration
--      creates the table with the exact column names (id, changed_at) that
--      step scans; NO second cron job, NO enforce_retention re-emission.
--
--   7. sync_pull(p_cursors): an eleventh per-profile key,
--      'day_entry_history', same tenant scoping and page contract as every
--      content table. PULL-ONLY: no sync_push parameter exists for it.
--
--   8. tombstone_profile_content(): the profile's history rows are
--      HARD-deleted (the #130 merge-events precedent -- after a content
--      wipe the feed rows describe content that no longer exists, and the
--      wipe is a privacy action), deliberately AFTER the function's
--      day_entries UPDATE: that update's own record_day_entry_change
--      trigger writes a fresh 'tombstoned' row per live entry, so deleting
--      first would leave them behind. Both callers (delete_profile_data /
--      delete_account_data) go through this helper, so neither is
--      re-emitted; the profiles/entry FK cascades cover the hard-purge
--      paths. A deleted caregiver's SURVIVING rows on a shared profile
--      (#17 R7) keep their changed_by_user_id: it carries NO auth.users
--      FK (a not-null column cannot take the merge-events tables' ON
--      DELETE SET NULL, and an FK would abort the account deletion) --
--      display attribution only, exactly like a denormalized display id.
--
--   9. reconcile_realtime_publication(): day_entry_history joins the
--      must-never-publish list (belt-and-suspenders: no column can carry
--      content, but a feed of metadata about a minor's health records is
--      still not websocket payload). The table also carries the standard
--      touch_sync_signal() AFTER trigger -- redundant with day_entries'
--      own signal in every current write path (history is only ever
--      written inside a transaction that touched day_entries), kept for
--      the same reason day_entry_merge_events keeps one: any future write
--      path must not silently lose the wake. The retention purge's DELETEs
--      fire it too -- a harmless no-op re-pull, the same posture the
--      merge-events purge already has.
--
--  10. sync_push is re-emitted from its current definition
--      (20260917120000_sync_push_derived_allowlists.sql, still the latest
--      on main) with exactly one change: the same-date resolver's two
--      discard branches also perform record_day_entry_merge_history() --
--      one 'merged_discard' row per discarded field, on the LOSING row's
--      id, attributed to the acting caller. THE MERGE RULE IS UNCHANGED.
--      The nine derived key allowlists are re-derived through
--      public.sync_push_payload_keys(t) and materialized at CREATE time
--      exactly as #181 established (AGENTS.md Migration Flow item 9) --
--      this table adds no synced column anywhere, so no allowlist changes
--      at all; the DO block below re-derives rather than hand-restates so
--      the sync_push_derived_allowlists_test.sql pinning stays true.
--
--  11. Local export: export_account_data() is deliberately untouched (the
--      #188/#128/#130 precedent -- client-synced tables are the local JSON
--      export's responsibility), and the LOCAL export is not extended
--      either: history rows are machine-written audit metadata, not user
--      state a backup needs to replay (#124's feed re-fills from the
--      server pull), and the #140 importer reading only known keys means
--      nothing round-trips either way.
--
--  12. observations (D-7): out of scope per the issue's own assumptions;
--      the same trigger shape is the documented follow-up for whichever
--      tracking-model issue implements it.
--
-- Coverage: supabase/tests/day_entry_history_test.sql (this table) and
-- supabase/tests/nightly_retention_job_test.sql (the now-live 90-day
-- purge step -- its old "table does not exist yet" structural guard is
-- replaced with real boundary coverage).

-- ---------------------------------------------------------------------------
-- 1. The content-free guarantee as a CHECKable predicate (#94 precedent)
-- ---------------------------------------------------------------------------

create or replace function public.is_valid_changed_fields(p_fields text[])
returns boolean
language sql
immutable
set search_path = ''
as $$
  select p_fields is not null
     and cardinality(p_fields) > 0
     and cardinality(p_fields) <= 10
     and not exists (
...
comment on function public.is_valid_changed_fields(text[]) is
  'Issue #170: the day_entry_history.changed_fields predicate -- every element must be a day_entries COLUMN NAME from the closed allowlist (never a value), the array non-empty and bounded by the allowlist''s own size. The structural backstop behind the table''s content-free guarantee, same shape as is_valid_tags_array (issue #94).';

revoke execute on function public.is_valid_changed_fields(text[]) from public, anon, authenticated;

-- ---------------------------------------------------------------------------
-- 2. public.day_entry_history
-- ---------------------------------------------------------------------------

create table public.day_entry_history (
  id text not null
    constraint day_entry_history_id_ulid_check
    check (id ~ '^[0-9A-HJKMNP-TV-Z]{26}$'),
  -- The day_entries row the change happened to. ON DELETE CASCADE: the
  -- only hard deletes of a day_entries row are content wipes and the
  -- 180-day tombstone purge (both past this table's own 90-day window),
  -- and a cascaded delete must never leave orphaned audit rows behind.
  entry_id text not null
    constraint day_entry_history_entry_id_ulid_check
    check (entry_id ~ '^[0-9A-HJKMNP-TV-Z]{26}$')
    references public.day_entries (id) on delete cascade,
  profile_id text not null
    references public.profiles (id) on delete cascade,
  -- Display attribution only, never a permission input and never an FK:
  -- the column is not null by this table's contract, and the merge-events
  -- tables' ON DELETE SET NULL shape cannot apply (an FK would abort
  -- account deletion for a caregiver whose logged rows survive, #17 R7).
  changed_by_user_id uuid not null,
  changed_at timestamptz not null default now(),
  change_kind text not null
    constraint day_entry_history_change_kind_check
    check (change_kind in ('logged', 'updated', 'tombstoned', 'merged_discard')),
  -- COLUMN NAMES ONLY, never values -- the whole point of the table. The
  -- CHECK is the structural guarantee (see is_valid_changed_fields above).
  changed_fields text[] not null
    constraint day_entry_history_changed_fields_check
    check (public.is_valid_changed_fields(changed_fields)),
  server_version bigint not null default 0,
  primary key (id)
);

comment on table public.day_entry_history is
  'Issue #170: one machine-written, content-free audit row per day_entries change (who/when/which column names/kind) -- the substrate #124''s shared activity feed reads. changed_fields carries day_entries COLUMN NAMES only, enforced against a closed allowlist by CHECK; written solely by the day_entries_record_history trigger and sync_push''s resolver (merged_discard); no client write path at all. Readable by every accepted guardian (RLS mirrors day_entries); hard-purged after 90 days by enforce_retention() and on a profile wipe by tombstone_profile_content().';

create index day_entry_history_profile_changed_at_idx
  on public.day_entry_history (profile_id, changed_at desc);
-- The incremental pull's (server_version) scan (the 20260913014000
-- per-table index, carried to every synced table since).
create index day_entry_history_server_version_idx
  on public.day_entry_history (server_version);
-- Retention scan support (the 90-day purge predicate), matching
-- day_entry_merge_events_created_at_idx's role in the same job.
create index day_entry_history_changed_at_idx
  on public.day_entry_history (changed_at);
-- The entry_id FK's cascade lookup (a purge/wipe bulk-deletes entries and
-- must find each entry's history rows without a sequential scan).
create index day_entry_history_entry_id_idx
  on public.day_entry_history (entry_id);

create trigger day_entry_history_set_server_version
  before insert or update on public.day_entry_history
  for each row execute function public.set_server_version();

-- Realtime: no publication membership (reconcile_realtime_publication
-- below). Redundant with day_entries' own signal in every current write
-- path (see header item 9) and kept for any future one.
create trigger day_entry_history_after_change_signal
  after insert or update or delete on public.day_entry_history
  for each row execute function public.touch_sync_signal();

-- ---------------------------------------------------------------------------
-- 3. Row-Level Security: reads mirror day_entries' guardian predicate;
--    writes have NO policy and NO grant at all -- the trigger below and
--    the resolver helper (both revoked from every client-facing role) are
--    the only writers, the exact posture notification_outbox carries.
-- ---------------------------------------------------------------------------

alter table public.day_entry_history enable row level security;
alter table public.day_entry_history force row level security;

create policy "day_entry_history_select_guardians" on public.day_entry_history
  for select to authenticated
  using (
    public.is_profile_guardian(profile_id, (select auth.uid()))
  );

revoke all on table public.day_entry_history from public, anon, authenticated;
grant select on table public.day_entry_history to authenticated;

-- ---------------------------------------------------------------------------
-- 4. The day_entries AFTER trigger. SECURITY DEFINER so it can insert into
--    day_entry_history (zero write policies, zero grants -- RLS denies the
--    row to every role it applies to, and the function owner is the same
--    owner every sibling SECURITY DEFINER writer in this schema runs as).
--    EXECUTE revoked from every client-facing role: the trigger fires on
--    day_entries writes, it is never called directly (the
--    enqueue_caregiver_alerts posture).
-- ---------------------------------------------------------------------------

create or replace function public.record_day_entry_change()
returns trigger
language plpgsql
security definer
set search_path = ''
as $$
declare
  -- The tracked columns -- exactly the set is_valid_changed_fields admits.
  -- Attribution/stamp columns (user_id, logged_by_user_id,
  -- last_modified_by_user_id, updated_at, server_version, created_at) are
  -- deliberately absent: a re-home or a no-op resave is not a content
  -- change, and the acting user already rides changed_by_user_id.
  c_tracked constant text[] := array[
    'local_date', 'tz', 'flow', 'tags', 'note', 'pms',
    'source', 'source_id', 'import_id', 'deleted_at'];
  v_changed_by uuid;
  v_kind text;
  v_fields text[];
begin
  v_changed_by := coalesce(new.last_modified_by_user_id,
                           new.logged_by_user_id,
                           new.user_id);
  -- No attribution at all means no real write path produced this row
  -- (see header item 3) -- record nothing rather than fail the write.
  ...
  if tg_op = 'INSERT' then
    if new.deleted_at is not null then
      -- Born dead: the resolver's FK-placeholder insert or an offline
      -- create-then-delete (header item 4). No live content ever existed
      -- server-side to disclose a change TO.
      return null;
    end if;
    v_kind := 'logged';
    -- An insert always establishes the date; each content column carrying
    -- a non-default value is named alongside it.
    v_fields := array['local_date']
      || case when new.tz is distinct from 'UTC' then array['tz'] else '{}'::text[] end
      || case when new.flow is distinct from 'none' then array['flow'] else '{}'::text[] end
      || case when new.tags is distinct from '[]'::jsonb then array['tags'] else '{}'::text[] end
      || case when new.note is not null then array['note'] else '{}'::text[] end
      || case when new.pms then array['pms'] else '{}'::text[] end;
  else
    if new.deleted_at is not null and old.deleted_at is null then
      v_kind := 'tombstoned';
      -- deleted_at always; plus the payload columns the tombstone
      -- actually cleared (only those that held something).
      v_fields := array['deleted_at']
        || case when new.flow is distinct from old.flow then array['flow'] else '{}'::text[] end
        || case when new.tags is distinct from old.tags then array['tags'] else '{}'::text[] end
        || case when new.note is distinct from old.note then array['note'] else '{}'::text[] end
        || case when new.pms is distinct from old.pms then array['pms'] else '{}'::text[] end;
    else
      v_kind := 'updated';
      -- Generic per-column diff over the tracked set, compared through
      -- to_jsonb so every column type renders uniformly.
      v_fields := array(
        select f from unnest(c_tracked) f
         where (to_jsonb(new) -> f) is distinct from (to_jsonb(old) -> f)
      );
      if cardinality(v_fields) = 0 then
        -- Nothing tracked changed (an attribution-only resave, a no-change
        -- tags union, an ownership-transfer re-home): no change, no row.
        return null;
      end if;
    end if;
  end if;
  ...
  insert into public.day_entry_history
    (id, entry_id, profile_id, changed_by_user_id, changed_at,
     change_kind, changed_fields)
  values
    (upper(substr(replace(gen_random_uuid()::text, '-', ''), 1, 26)),
     new.id, new.profile_id, v_changed_by, clock_timestamp(),
     v_kind, v_fields);
  return null; -- AFTER trigger; return value is ignored.
end;
$$;

comment on function public.record_day_entry_change() is
  'Issue #170: the day_entries AFTER INSERT/UPDATE trigger writing one content-free day_entry_history row per change -- logged (insert live), updated (tracked-column diff), tombstoned (live->dead). Attribution comes from the row sync_push just stamped (coalesce(last_modified_by_user_id, logged_by_user_id, user_id)). Born-dead inserts and attribution-only updates record nothing (see the migration header). SECURITY DEFINER (the table admits no other writer); never called directly.';

revoke execute on function public.record_day_entry_change() from public, anon, authenticated;

create trigger day_entries_record_history
  after insert or update on public.day_entries
  for each row execute function public.record_day_entry_change();

-- ---------------------------------------------------------------------------
-- 5. The resolver's merged_discard emission helper. SECURITY INVOKER (it
--    only ever runs inside sync_push's own SECURITY DEFINER context, as
--    the function owner), fully qualified, EXECUTE revoked from every
--    client-facing role -- the record_day_entry_merge_discard shape.
-- ---------------------------------------------------------------------------

create or replace function public.record_day_entry_merge_history(
  p_profile_id text,
  p_entry_id text,
  p_field text,
  p_changed_by uuid
...
begin
  ...
  values
    (upper(substr(replace(gen_random_uuid()::text, '-', ''), 1, 26)),
     p_entry_id, p_profile_id, p_changed_by, clock_timestamp(),
     'merged_discard', array[p_field]);
end;
$$;

comment on function public.record_day_entry_merge_history(text, text, text, uuid) is
  'Issue #170: records one content-free merged_discard history row for a same-date merge discard -- the LOSING row''s id and the discarded field NAME only (day_entry_merge_events carries the recoverable text; this carries the feed entry). Called only from sync_push''s same-date resolver branches, next to record_day_entry_merge_discard.';

revoke execute on function public.record_day_entry_merge_history(text, text, text, uuid) from public, anon, authenticated;

-- ---------------------------------------------------------------------------
-- 6. sync_push: re-emitted from its current definition
--    (20260917120000_sync_push_derived_allowlists.sql, unchanged since)
--    with exactly the Issue #170 change its own comments name -- the two
--    same-date resolver branches additionally perform
--    record_day_entry_merge_history() next to each existing
--    record_day_entry_merge_discard() call. Everything else (the nine
--    DERIVED allowlists included) carries forward unchanged; the allowlist
--    constants are re-derived through public.sync_push_payload_keys(t) and
--    materialized at CREATE time, exactly the #181 DO-block recipe
--    (AGENTS.md Migration Flow item 9) -- this issue adds no synced column
--    anywhere, so the derivation itself is untouched and
--    sync_push_derived_allowlists_test.sql keeps pinning it.
--
--    The signature is unchanged (nine jsonb parameters, the same
--    defaults), so this is a plain CREATE OR REPLACE: no overload dance,
--    and the existing EXECUTE grants stand.
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- The 20260917120000 statement verbatim except the nine array
  -- declarations (placeholder tokens) and the Issue #170 resolver edits.
  v_body := $ddl$
...
declare
  ...
  -- #257 adds the ninth (p_tag_registry), so the cap grows with it to
  -- 4500 (= 9 * c_max_rows) -- still exactly one full batch per array,
  ...
  c_max_total_rows constant integer := 4500;
  ...
  -- Issue #181: the nine per-table key allowlists below are DERIVED, not
...
begin
  ...
  if jsonb_array_length(p_profiles) + jsonb_array_length(p_day_entries)
     ...
     + jsonb_array_length(p_tag_registry) > c_max_total_rows then
...
-- ---------------------------------------------------------------------------
-- 7. sync_pull: re-emitted from its current definition
--    (20260917000000_profile_tag_registry.sql, unchanged since) with the
--    eleventh per-profile key, 'day_entry_history' (header item 7).
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- Issue #257: profile_tag_registry rides the same per-profile scoping
  -- as every content table above (the tenth per-profile key -- a missing
  -- key in this response would make the client's pull decoder drop its
  -- whole cache, so the client side grows with the server, atomically).
  v_cursor := coalesce((p_cursors ->> 'profile_tag_registry')::bigint, 0);
  v_result := v_result || jsonb_build_object('profile_tag_registry', (
    select coalesce(jsonb_agg(to_jsonb(t)), '[]'::jsonb) from (
      select * from public.profile_tag_registry
       where profile_id = any(v_profile_ids)
         and server_version > v_cursor
       order by server_version
       limit c_page_size
  ...
  -- Issue #170: day_entry_history rides the same per-profile scoping as
  -- every content table above -- the eleventh per-profile key, and
  -- PULL-ONLY: the table has no sync_push parameter (rows are written by
  -- the day_entries trigger and the resolver), so this key plus the RLS
  -- select policy are its entire client contract.
  v_cursor := coalesce((p_cursors ->> 'day_entry_history')::bigint, 0);
  v_result := v_result || jsonb_build_object('day_entry_history', (
    select coalesce(jsonb_agg(to_jsonb(t)), '[]'::jsonb) from (
      select * from public.day_entry_history
       where profile_id = any(v_profile_ids)
         and server_version > v_cursor
       order by server_version
       limit c_page_size
...
--    (20260917000000_profile_tag_registry.sql, unchanged since) with the
--    day_entry_history hard-delete step + count (header item 8).
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 9. reconcile_realtime_publication(): re-emitted from its current
--    definition (20260917000000_profile_tag_registry.sql, unchanged
--    since) with day_entry_history on the must-never-publish list
--    (header item 9).
-- ---------------------------------------------------------------------------
...
begin
  ...
  if v_all_tables then
    raise exception
      ...
      'public.day_entry_merge_events, public.profile_tag_registry, and '
      'public.day_entry_history '
...
comment on function public.reconcile_realtime_publication() is
  ...
  'profile_tag_registry/day_entry_history '
  ...
  '#170 added day_entry_history), drift '
