...
- **Backup Exclusion:** The local database is excluded from both iCloud/device backup (iOS) and cloud backup and device-to-device transfer (Android), and on iOS is marked with the strongest file-protection class so it is unreadable while the device is locked.
...
- **September 8, 2026:** Strengthened the "Protected at Rest" principle and the "Device Encryption" security protection (issue #244): the local database moved out of the directory backed up by iCloud/device backup by default, and on iOS is now additionally excluded from backup and marked with the platform's strongest file-protection class. Android gained the equivalent device-to-device-transfer exclusion alongside its existing backup-off setting.
